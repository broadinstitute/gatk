...
**input_vcf_fofn** - A FOFN (file of file names) of VCFs generated by `GvsExtractCallSet`. These need not be subsetted down to the chromosome.
These should be the full cloud paths to the VCFs, not just the file names.
...
```
gs://<long prefix with workspace id, submission id, etc>/call-ExtractTask/shard-0/0000000000-quickit.vcf.gz
```

Then all of the output VCFs can be selected into a FOFN:
