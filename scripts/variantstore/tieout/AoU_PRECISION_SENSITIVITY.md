...
1. Truth data `.bed`, `.vcf`, and `.tbi` files for each control.
1. "Storage Object View" access granted for your @pmi-ops proxy group on the `gs://broad-dsp-spec-ops/gvs/truth` directory

## Precision and Sensitivity
- This workflow does not use the Terra Data Entity Model to run, so be sure to select the `Run workflow with inputs defined by file paths` workflow submission option.
- inputs:
  - The values for `call_set_identifier`, `dataset_name`, `filter_set_name`, and `project_id` should be the same value used for other callset workflows.
