...
This Docker image should be built on an x86 machine such as a GCE or Azure VM. Attempts to build this on an Apple Silicon
Mac resulted in frequent random segmentation faults and a final image that was mysteriously missing critical dependencies.

Steps to build:

- Clone the Ensembl VEP repo [here](https://github.com/Ensembl/ensembl-vep#) and make sure the appropriate branch is checked out.
For Ensembl 115 this branch is `releases/115`, which at the time of this writing also happens to be the default branch.
...
- Run the script. No arguments are required, the script should build and push the image and print out its tag to standard output.
