...
task EnsureVatTableHasVariants {
    ...
    command <<<
        ...
        bq --apilog=stderr query --nouse_legacy_sql --project_id=~{project_id} --format=csv 'SELECT COUNT (DISTINCT vid) AS count FROM `~{fq_vat_table}`' > bq_variant_count.csv
...
task SpotCheckForExpectedTranscripts {
    ...
    command <<<
        ...
        bq --apilog=stderr query --nouse_legacy_sql --project_id=~{project_id} --format=csv 'SELECT
            contig,
            position,
            vid,
            gene_symbol,
...
task SchemaNoNullRequiredFields {
    ...
    command <<<
        ...
        bq --apilog=stderr query --nouse_legacy_sql --project_id=~{project_id} --format=csv
        'SELECT
            contig,
            position,
            vid,
...
task DuplicateAnnotations {
    ...
    command <<<
        ...
        bq --apilog=stderr query --nouse_legacy_sql --project_id=~{query_project_id} --format=csv '
