...
1. `GvsExtractCallset` / `GvsExtractCallsetPgenMerged` workflows ("small callset" Exome, Clinvar, and ACAF threshold extracts in VCF and PGEN formats respectively)
    ...
    - The ACAF interval list appears to be particularly challenging for the GVS extract workflows. For both PGEN and VCF extracts of ACAF the following considerations apply:
