...
task LoadArrays {
  ...
  command <<<
    ...
    if [ $NUM_METADATA_FILES -gt 0 ]; then
      set +e
      bq --apilog=stderr ls --project_id ~{project_id} ~{dataset_name} > /dev/null
      set -e
      if [ $? -ne 0 ]; then
        echo "making dataset ~{dataset_name}"
        bq --apilog=stderr mk --project_id=~{project_id} ~{dataset_name}
      fi
      set +e
      bq --apilog=stderr show --project_id ~{project_id} $SAMPLE_LIST_TABLE > /dev/null
      BQ_SHOW_RC=$?
      set -e
      if [ $BQ_SHOW_RC -ne 0 ]; then
        echo "making table $SAMPLE_LIST_TABLE"
        bq --apilog=stderr --location=US mk --project_id=~{project_id} $SAMPLE_LIST_TABLE ~{sample_list_schema}
      ...
      if [ ~{load} = true ]; then
        bq --apilog=stderr load --location=US --project_id=~{project_id} --skip_leading_rows=1 --null_marker="null" --source_format=CSV -F "\t" $SAMPLE_LIST_TABLE $SAMPLE_DIR$METADATA_FILES ~{sample_list_schema}
    ...
    if [ $NUM_RAW_FILES -gt 0 ]; then
      ...
      if [ $? -ne 0 ]; then
        ...
        bq --apilog=stderr --location=US mk --range_partitioning=$PARTITION_FIELD,$PARTITION_START,$PARTITION_END,$PARTITION_STEP \
          --project_id=~{project_id} $TABLE ~{raw_schema}
        #TODO: add a Google Storage Transfer for the table when we make it.
      fi
      if [ ~{load} = true ]; then
        bq --apilog=stderr load --location=US --project_id=~{project_id} --skip_leading_rows=1 --null_marker="null" --source_format=CSV -F "\t" $TABLE $RAW_DIR$RAW_FILES ~{raw_schema}
