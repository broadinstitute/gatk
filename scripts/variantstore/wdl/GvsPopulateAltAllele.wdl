...
task GetVetTableNames {
  ...
  command <<<
    ...
    bq --apilog=false query --project_id=~{project_id} --format=csv --use_legacy_sql=false ~{bq_labels} 'SELECT table_name FROM `~{project_id}.~{dataset_name}.INFORMATION_SCHEMA.TABLES` WHERE table_name LIKE "vet_%" AND CAST(SUBSTRING(table_name, length("vet_") + 1) AS INT64) >= ${min_vat_table_num}' > vet_tables.csv
