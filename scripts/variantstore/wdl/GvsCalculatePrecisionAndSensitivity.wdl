...
workflow GvsCalculatePrecisionAndSensitivity {
  input {
    ...
    Boolean exclude_sex_chromosomes = false
  }
  ...
  parameter_meta {
    ...
    chromosomes: "The chromosome(s) on which to run the analysis of Precision and Sensitivity. The default value for this is `['chr20']`. If it is set to `[]` then the analysis will be run across *all* chromosomes."
  ...
  Array[String]? contigs = if (chromosomes == [] && exclude_sex_chromosomes == false)
                   then none
                   else
                    if (chromosomes == [] && exclude_sex_chromosomes == true)
                    then ["chr1","chr2","chr3","chr4","chr5","chr6","chr7","chr8","chr9","chr10","chr11","chr12","chr13","chr14","chr15","chr16","chr17","chr18","chr19","chr20","chr21","chr22"]
                    else chromosomes
...
task Add_AS_MAX_VQS_SCORE_ToVcf {
  input {
    ...
    String docker = "us.gcr.io/broad-dsde-methods/variantstore:2023-06-01-alpine"
    Int cpu = 1
    Int memory_mb = 3500
    Int disk_size_gb = ceil(2*size(input_vcf, "GiB")) + 50
  }
