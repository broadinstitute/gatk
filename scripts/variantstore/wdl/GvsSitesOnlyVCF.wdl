...
task ExtractAnAcAfFromVCF {
    ...
    command <<<
        ...
        bcftools norm -m- ~{local_input_vcf} > normalized.vcf

        bcftools norm -f Homo_sapiens_assembly38.fasta normalized.vcf  | bcftools plugin fill-tags  -- -S ~{subpopulation_sample_list} -t AC,AF,AN,AC_het,AC_hom | bcftools query -f \
