...
task WithdrawSamples {
  ...
  command <<<
    ...
    bq --apilog=stderr --project_id=~{project_id} mk --expiration=86400 ~{dataset_name}.current_callset_samples "sample_name:STRING"
    # populate the temp table
    bq --apilog=stderr load --project_id=~{project_id} -F "tab" ~{dataset_name}.current_callset_samples sample_names.tsv

    # join on the temp table to figure out which samples should be marked as withdrawn
    bq --apilog=stderr --project_id=~{project_id} query --format=csv --use_legacy_sql=false \
