...
def populate_alt_allele_table(query_project, vet_table_name, fq_dataset, sa_key_path):
    ...
    query_with = f"""INSERT INTO `{fq_dataset}.alt_allele`
                WITH 
                  position1 as (select * from `{fq_vet_table}` WHERE call_GT IN ('0/1', '1/0', '1/1', '0|1', '1|0', '1|1', '0/2', '0|2','2/0', '2|0') AND sample_id NOT IN (SELECT sample_id from {fq_dataset}.sample_info WHERE withdrawn IS NOT NULL)),
                  position2 as (select * from `{fq_vet_table}` WHERE call_GT IN ('1/2', '1|2', '2/1', '2|1') AND sample_id NOT IN (SELECT sample_id from {fq_dataset}.sample_info WHERE withdrawn IS NOT NULL))"""
