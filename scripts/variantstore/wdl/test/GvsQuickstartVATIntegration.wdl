...
workflow GvsQuickstartVATIntegration {
    ...
    call AssertKindaIdenticalOutputs {
        input:
...
# Note - the tsv generated by the export of the VAT table is not consistently sorted (I think it's sorted by vid, but not by transcript?)
# So, a diff or cmp doesn't work. In this method I just verify that the files have the same counts (from wc)
task AssertKindaIdenticalOutputs {
    input {
        File actual_file
        File expected_file
        String gatk_docker
