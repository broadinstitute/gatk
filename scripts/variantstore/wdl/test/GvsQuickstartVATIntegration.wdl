...
workflow GvsQuickstartVATIntegration {
    ...
    File? vds_path = if (use_vds_as_input) then truth_data_prefix + "gvs_export.vds" else none
    File? sites_only_vcf = if (!use_vds_as_input) then truth_data_prefix + "quickstart_sites_only.vcf.bgz" else none
