...
task GetBQTablesMaxLastModifiedTimestamp {
  ...
  command <<<
    ...
    bq --apilog=stderr --project_id=~{query_project} query --format=csv --use_legacy_sql=false \
...
task BuildGATKJarAndCreateDataset {
  ...
  command <<<
    ...
    bq --apilog=stderr mk --project_id="gvs-internal" "$dataset"

    # add labels for DSP Cloud Cost Control Labeling and Reporting
    bq --apilog=stderr update --set_label service:gvs gvs-internal:$dataset
    bq --apilog=stderr update --set_label team:variants gvs-internal:$dataset
    bq --apilog=stderr update --set_label environment:dev gvs-internal:$dataset
    bq --apilog=stderr update --set_label managedby:build_gatk_jar_and_create_dataset gvs-internal:$dataset
...
task IsVQSRLite {
  ...
  command <<<
    ...
    bq --apilog=stderr query --project_id='~{project_id}' --format=csv --use_legacy_sql=false ~{bq_labels} \
