...
public class SimpleGermlineTaggerUnitTest extends GATKBaseTest {
    ...
    public void testSimpleTagging(List<AnnotatedInterval> tumorSegments, List<AnnotatedInterval> normalSegments,
                                  ...
                                  final double mafMaxThreshold, final int cnLoHCheckMaxSize) {
        List<AnnotatedInterval> testResult = null;
        if (!isCNLoHCheck) {
            testResult = SimpleGermlineTagger.tagTumorSegmentsWithGermlineActivity(tumorSegments,
                    normalSegments, CALL_ANNOTATION,
                    ReferenceUtils.loadFastaDictionary(new File(ReferenceUtils.getFastaDictionaryFileName(REF))),
                    TEST_GERMLINE_TAGGING_ANNOTATION, padding, reciprocalThreshold);
        } else {
            testResult = SimpleGermlineTagger.tagTumorSegmentsWithGermlineActivity(tumorSegments,
                    normalSegments, CALL_ANNOTATION,
                    ReferenceUtils.loadFastaDictionary(new File(ReferenceUtils.getFastaDictionaryFileName(REF))),
                    TEST_GERMLINE_TAGGING_ANNOTATION, padding, reciprocalThreshold,
                    mafMaxThreshold, cnLoHCheckMaxSize, MAF_HI_ANNOTATION, MAF_LO_ANNOTATION);
        }
        Assert.assertEquals(testResult, gt);
    }

    /**
     * Note that this tests whether the CNLoH parameters are necessary if not being used.  Current requirement is that
     *  those parameters are not necessary if not being used.
     */
    ...
    public Object[][] createSimpleTests() {
        ...
        /* END MAF NULL CASES */
        // The germline tagging should happen due to the CR, even though the MAF imbalance is present.
                {
                        Arrays.asList(
                                // The third segment should not get tagged.  The first should get tagged, because the reciprocal overlap holds, due to low reciprocal threshold.
                                new AnnotatedInterval(new SimpleInterval("1", 100, 200), Maps.newTreeMap(ImmutableSortedMap.of(MAF_HI_ANNOTATION, "0.495", MAF_LO_ANNOTATION, "0.49", CALL_ANNOTATION, "+"))),
