...
public class MergeAnnotatedRegionsIntegrationTest extends CommandLineProgramTest {
    ...
    private static final String SIMPLE_TEST_FILE = TEST_SUB_DIR + "merge-annotated-regions-simple-test.tsv";
    private static final String REF = hg19MiniReference;

    @Test
    public void basicTest() throws IOException {
        // This test is a bit more like the real world
        final File outputFile = File.createTempFile("mergeannotatedregions", ".tsv");
        ...
        Assert.assertEquals(collection.getRecords().get(0), new SimpleAnnotatedGenomicRegion(new SimpleInterval("1", 100, 105),
                ImmutableSortedMap.of("Num_Probes", "100", "Segment_Mean", "-0.03", "Segment_Call", "0")));
        Assert.assertEquals(collection.getRecords().get(1), new SimpleAnnotatedGenomicRegion(new SimpleInterval("1", 525, 2305),
                ImmutableSortedMap.of("Num_Probes", "200", "Segment_Mean", "-0.10__-0.76", "Segment_Call", "-__0")));
        Assert.assertEquals(collection.getRecords().get(2), new SimpleAnnotatedGenomicRegion(new SimpleInterval("1", 2306, 2605),
                ImmutableSortedMap.of("Num_Probes", "200__500", "Segment_Mean", "-0.60", "Segment_Call", "-")));
        Assert.assertEquals(collection.getRecords().get(3), new SimpleAnnotatedGenomicRegion(new SimpleInterval("2", 525, 1097),
                ImmutableSortedMap.of("Num_Probes", "200", "Segment_Mean", "-0.76", "Segment_Call", "-")));
        Assert.assertEquals(collection.getRecords().get(4), new SimpleAnnotatedGenomicRegion(new SimpleInterval("2", 1098, 2305),
