...
public class SelectVariantsMultiThreadedIntegrationTest extends CommandLineProgramTest {

    /**
     * Default number of worker threads to exercise in the multi-threaded test runs. Picked to be
     * comfortably more than 1 but well below the typical CI machine size, so ordering bugs surface
     * but the suite stays fast.
     */
    private static final int TEST_THREADS = 4;

    /**
     * Reuse the existing SelectVariants test-resource directory so this duplicated test class doesn't
     * require its own copy of all fixture files.
     */
    @Override
    ...
    /**
     * Variant of {@link #baseTestString(String, String)} that forces serial execution. Required for
     * tests exercising features the multi-threaded walker cannot run concurrently (currently
     * {@code --discordance}, {@code --concordance}, and {@code --mendelian-violation}; see the guard
     * in {@code SelectVariantsMultiThreaded.onTraversalStart}).
     */
    private static String baseTestStringSerial(String args, String testFile) {
        return " --variant " + testFile
                    + " -O %s "
                    + " --" + StandardArgumentDefinitions.ADD_OUTPUT_VCF_COMMANDLINE +" false "
                    + " --threads 1 "
    ...
    public void testDiscordance() throws IOException {
        ...
        final IntegrationTestSpec spec = new IntegrationTestSpec(
                baseTestStringSerial(" -sn NA11992 " // not present in discordance track
    ...
    public void testConcordance()  throws IOException {
        ...
        final IntegrationTestSpec spec = new IntegrationTestSpec(
                baseTestStringSerial(" -sn NA11894 -conc " + concordanceFile  + " --lenient ", testFile),
    ...
    public void testMendelianViolationSelection() throws IOException {
        ...
        final IntegrationTestSpec spec = new IntegrationTestSpec(
                baseTestStringSerial(" -ped " + pedFile + " --mendelian-violation --mendelian-violation-qual-threshold 0 ", testFile),
    ...
    public void testInvertMendelianViolationSelection() throws IOException {
        ...
        final IntegrationTestSpec spec = new IntegrationTestSpec(
                baseTestStringSerial(" --mendelian-violation --mendelian-violation-qual-threshold 0 --invert-mendelian-violation -ped " + pedFile, testFile),
