...
public final class VariantFiltrationMultiThreadedIntegrationTest extends CommandLineProgramTest {

    /** Worker thread count to exercise in the multi-threaded test runs. */
    private static final int TEST_THREADS = 4;

    /**
     * Reuse the existing VariantFiltration test-resource directory so this duplicated test class
     * doesn't need its own copy of fixtures/expected outputs.
     */
    @Override
     ...
     * {@code --mask} and {@code --cluster-window-size}; see the guard in
     * {@code VariantFiltrationMultiThreaded.onTraversalStart}).
     */
    public String baseTestStringSerial(final String vcf, final String options) {
        final String file = getToolTestDataDir() + vcf;
        return "--variant " + file + " " + options + " -O %s" + " -R " + hg19_chr1_1M_Reference
                + " --" + StandardArgumentDefinitions.ADD_OUTPUT_VCF_COMMANDLINE + " false"
                + " --threads 1";
    ...
    public void testClusteredSnps() throws IOException {
        final IntegrationTestSpec spec = new IntegrationTestSpec(
        baseTestStringSerial("vcfexample2.vcf", " -cluster-window-size 10 "),
    ...
    public void testMask(final String maskName, final String mask, final String expected) throws IOException {
        final IntegrationTestSpec spec = new IntegrationTestSpec(
                baseTestStringSerial("vcfexample2.vcf", " -mask-name " + maskName + " " + mask),
    ...
    public void testMaskWithFilters(final String maskName, final String mask, final String expected) throws IOException {
        final IntegrationTestSpec spec = new IntegrationTestSpec(
                baseTestStringSerial("filtered.vcf", " -mask-name " + maskName + " " + mask),
                Arrays.asList(getToolTestDataDir() + "expected/" + expected)
        );
    ...
    public void testMaskReversed() throws IOException {
        final IntegrationTestSpec spec = new IntegrationTestSpec(
                baseTestStringSerial("vcfexample2.vcf", " -mask-name outsideGoodSites -filter-not-in-mask --mask:BED " + getToolTestDataDir() + "goodMask.bed"),
    ...
    public void testMaskDescription() throws IOException {
        ...
        args.add("V", getTestFile("vcfexample2.vcf"))
                ...
                .add("threads", "1");  // --mask is not safe with --threads > 1 in the experimental MT version.
