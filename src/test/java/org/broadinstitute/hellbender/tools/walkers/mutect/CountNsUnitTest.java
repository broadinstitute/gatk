...
public class CountNsUnitTest {
    ...
    private final int numChromosomes = 2; // create chromosome 0 and chromosome 1
    private final int startingChromosome = 1;
    private final int chromosomeSize = 1000;
    private final SAMFileHeader SAM_HEADER = ArtificialReadUtils.createArtificialSamHeader(numChromosomes, startingChromosome, chromosomeSize);

    private final int chromosomeIndex = 1;
    private final long variantSite = 104;
    private final int alignmentStart = 100;
    private final int readLength = 9;

    private final List<Allele> alleles = Arrays.asList(Allele.create((byte) 'C', true), Allele.create((byte) 'A', false));
    private final AlleleList<Allele> alleleList = new IndexedAlleleList<>(alleles);
    private final VariantContext vc = new VariantContextBuilder("source", Integer.toString(chromosomeIndex), variantSite, variantSite, alleles)
            .attribute(GATKVCFConstants.TUMOR_LOD_KEY, new double[]{ 6.4 }).make();

    private final String sampleName = "SamThreetree";
    private final SampleList sampleList = new IndexedSampleList(sampleName);

    private final byte baseq = 30;
    private final int mapq = 60;
    ...
    public void testSNP() throws IOException {
        ...
        for (int i = 0; i < numAltReads; i++) {
            ...
            if(i == 5 || i == 8 || i == 9) {
                ...
                read = ArtificialReadUtils.createArtificialRead(SAM_HEADER, "Read" + i, chromosomeIndex, alignmentStart,
                        "CCCCNCCCC".getBytes(), quals, "9M");
            } else {
                read = ArtificialReadUtils.createArtificialRead(SAM_HEADER, "Read" + i, chromosomeIndex, alignmentStart,
        ...
        final CountNs countNsAnnotation = new CountNs();
        final GenotypeBuilder genotypeBuilder = new GenotypeBuilder(sampleName);
        final Genotype genotype = genotypeBuilder.make();

        countNsAnnotation.annotate(null, vc, genotype, genotypeBuilder, likelihoods);

        Assert.assertEquals((int) genotype.getExtendedAttribute(GATKVCFConstants.N_BASE_COUNT_KEY), 3);
    }
