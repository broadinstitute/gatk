...
public class DownsampleByDuplicateSetIntegrationTest extends CommandLineProgramTest {
    ...
    public void testMatesAreTogether() {
        ...
        try (final ReadsDataSource readsDataSource = new ReadsPathDataSource(new GATKPath(out))) {
    ...
    public void testDownsampleFraction() {
        ...
        for (final double downsampleRate : Arrays.asList(0.0, 0.1, 0.3, 0.5, 1.0)) {
            final ArgumentsBuilder args = new ArgumentsBuilder()
                .add("I", NA12878_GROUPED)
            ...
            try (final ReadsDataSource originalBam = new ReadsPathDataSource(new GATKPath(NA12878_GROUPED));
                 final ReadsDataSource downsampledBam = new ReadsPathDataSource(new GATKPath(out))) {
    ...
    private static int countDuplicateSets(final ReadsDataSource readsDataSource) {
        int count = 0;
        String currentMoleculeId = ""; // Note we are duplex aware: 12/A different from 12/B

        for (final GATKRead read : readsDataSource) {
