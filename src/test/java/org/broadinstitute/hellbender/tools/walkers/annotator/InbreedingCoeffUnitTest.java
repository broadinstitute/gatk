...
public final class InbreedingCoeffUnitTest extends GATKBaseTest {
    ...
    private final int[] het4 = {520,0,820};
    ...
    public void compareOldWithReblocked() {
        ...
        for (int i = 0; i < 61; i++) {
            oldGTs.add(makeG("s" + i, Aref, Aref, confidentHomRefPLs));
        }
        oldGTs.add(makeG("het1", Aref, T, het1));
        oldGTs.add(makeG("het2", Aref, T, het2));
        oldGTs.add(makeG("het3", Aref, T, het3));
        ...
        newGTs.add(makeG("het3", Aref, T, het3));
