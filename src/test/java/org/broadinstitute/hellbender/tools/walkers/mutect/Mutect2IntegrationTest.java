...
public class Mutect2IntegrationTest extends CommandLineProgramTest {
    ...
    public void testPon(final File tumorBam, final String tumorSample, final File normalBam, final String normalSample) throws Exception {
        ...
        final long numVariants = Utils.streamVcf(filteredVcf)
    ...
    public void testTumorNormal() throws Exception {
        ...
        runCommandLine(args);
        final long numVariants = Utils.streamVcf(outputVcf).count();
    ...
    public void testTumorOnly() throws Exception {
        ...
        final long numVariantsBeforeFiltering = Utils.streamVcf(unfilteredVcf).count();

        final long numVariantsPassingFilters = Utils.streamVcf(filteredVcf)
                .filter(vc -> vc.getFilters().isEmpty()).count();
    ...
    public void testGivenAllelesMode() throws Exception {
        ...
        final Map<Integer, List<Allele>> altAllelesByPosition = Utils.streamVcf(unfilteredVcf)
    ...
    public void testContaminationFilter() throws Exception {
        ...
        final Set<VariantContext> variantsFilteredAtZeroPct =
                Utils.streamVcf(filteredVcfNoContamination)
        ...
        final Set<VariantContext> variantsFilteredAtFivePct =
                Utils.streamVcf(filteredVcfFivePctContamination)
                        .filter(vc -> vc.getFilters().contains(GATKVCFConstants.CONTAMINATION_FILTER_NAME))
                        .collect(Collectors.toSet());

        final Set<VariantContext> variantsFilteredAtTenPct =
                Utils.streamVcf(filteredVcfTenPctContamination)
                        .filter(vc -> vc.getFilters().contains(GATKVCFConstants.CONTAMINATION_FILTER_NAME))
                        .collect(Collectors.toSet());
        ...
        final List<VariantContext> missedObviousVariantsAtTenPercent = Utils.streamVcf(filteredVcfTenPctContamination)
                .filter(vc -> !vc.getFilters().contains(GATKVCFConstants.CONTAMINATION_FILTER_NAME))
        ...
        final List<VariantContext> highAlleleFractionFilteredVariantsAtFivePercent = Utils.streamVcf(filteredVcfFivePctContamination)
                .filter(vc -> vc.getFilters().contains(GATKVCFConstants.CONTAMINATION_FILTER_NAME))
                .filter(VariantContext::isBiallelic)
                .filter(vc -> {
                    final int[] AD = vc.getGenotype(0).getAD();
