...
public class ConvertACNVResultsIntegrationTest extends CommandLineProgramTest {
    ...
    public void testRun(final File tumorAllelicCountsFile) {
        ...
        arguments.add("INFO");
        arguments.add("-" + ConvertACNVResults.OUTPUT_DIR_SHORT_NAME);
        arguments.add(outputDir.getAbsolutePath());
        arguments.add("-" + ConvertACNVResults.NUM_ITERATIONS_SHORT_NAME);
        arguments.add("3");

        runCommandLine(arguments);

        final String gatkCnvFilename = createFilename(outputDir, ConvertACNVResults.GATK_SEG_FILE_TAG + ".seg", SEGMENT_FILE);
        Assert.assertTrue(new File(gatkCnvFilename).exists());

        final String acsFilename = createFilename(outputDir, ConvertACNVResults.CGA_ACS_SEG_FILE_TAG + ".seg", SEGMENT_FILE);
        Assert.assertTrue(new File(acsFilename).exists());

        final String balancedSegFilename = createFilename(outputDir, ConvertACNVResults.BALANCED_SEG_FILE_TAG + ".seg", SEGMENT_FILE);
        Assert.assertTrue(new File(balancedSegFilename).exists());

        final String titanTNFilename = createFilename(outputDir, ConvertACNVResults.TITAN_TN_FILE_TAG + ".tsv", SEGMENT_FILE);
        Assert.assertTrue(new File(titanTNFilename).exists());

        final String titanHetFilename = createFilename(outputDir, ConvertACNVResults.TITAN_HET_FILE_TAG + ".tsv", SEGMENT_FILE);
