...
public class TargetCoverageSexGenotyperIntegrationTest extends CommandLineProgramTest {
    ...
    /**
     * A user can provide a genotyping target test; the target list can include bait counts as a means to improve
     * modeling bait biases
     */
    @Test
    ...
    /**
     * Certain targets can be excluded by providing a target list to the tool. If only a fraction of targets are excluded,
     * inferred sex genotypes are not affected (though the likelihoods change).
     *
    ...
    /**
     * Certain targets can also be excluded via command line. If only a fraction of targets are excluded,
     * inferred sex genotypes are not affected (though the likelihoods change).
     */
    @Test
    ...
    /**
     * Bait counts change sex genotype likelihoods
     */
    ...
    /**
     * We are very likely to determine the correct sex genotype even without ChrY coverage (just based on ChrX ploidy
     * difference between XX and XY). This, however, requires a "reliable" model. At least, taking into account bait count
     * bias is necessary (see the next test).
     */
    @Test
    ...
    /**
     * Without ChrY coverage, sex genotyping is unreliable. In particular, without taking into account bait count bias,
     * the model is biased toward higher X-ploidy sex genotypes (here, XX).
     */
    @Test
