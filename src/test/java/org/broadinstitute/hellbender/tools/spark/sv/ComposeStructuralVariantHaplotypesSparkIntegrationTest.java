...
public class ComposeStructuralVariantHaplotypesSparkIntegrationTest extends CommandLineProgramTest {
    ...
    public void testComposeSVContigs() throws IOException {
        ...
        try {
            outputBamFile = File.createTempFile("test-output", ".bam");
            alignedOutputBamFile = File.createTempFile("test-output-aln", ".bam");
            final IntegrationTestSpec spec = new IntegrationTestSpec(String.format("-%s %s -%s %s -%s %s -%s %s -%s %s -%s %s",
    ...
    private static void assertReadsAreEqual(final SAMRecord expectedRead, final SAMRecord actualRead) {
        ...
        Assert.assertEquals(expectedRead.getStart(), actualRead.getStart());
        Assert.assertEquals(composeReadAttributeSortedList(actualRead), composeReadAttributeSortedList(expectedRead),
                composeReadAttributeSortedList(actualRead).stream().map(x -> x.getLeft()).collect(Collectors.joining(", ")) + " |||| " +
                        composeReadAttributeSortedList(expectedRead).stream().map(x -> x.getLeft()).collect(Collectors.joining(", ")));
    }

    private static List<Pair<String, Object>> composeReadAttributeSortedList(final SAMRecord expectedRead) {
