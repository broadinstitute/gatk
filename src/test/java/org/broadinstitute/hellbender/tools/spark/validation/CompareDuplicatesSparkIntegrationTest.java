...
public class CompareDuplicatesSparkIntegrationTest extends CommandLineProgramTest {
    ...
    public void testOutputFile() throws Exception {
        ...
        for (int i = 0; i < bam1Diffs.getRight().size(); i++) {
            Assert.assertEquals(bam1Diffs.getRight().get(i).getName(), bam2Diffs.getRight().get(i).getName());
            Assert.assertEquals(bam1Diffs.getRight().get(i).getStart(), bam2Diffs.getRight().get(i).getStart());
            Assert.assertFalse(bam2Diffs.getRight().get(i).isDuplicate());
        }
    }
}