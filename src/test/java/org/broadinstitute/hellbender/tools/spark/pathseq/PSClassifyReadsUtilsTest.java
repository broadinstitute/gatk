...
public class PSClassifyReadsUtilsTest extends CommandLineProgramTest {
    ...
    @Test
    public void testWrapInIterable() {
        final Integer obj = new Integer(5);
        final Iterable<Integer> iter = PSClassifyReadsUtils.wrapInIterable(obj);
        Assert.assertTrue(iter.iterator().hasNext());
        Assert.assertEquals(iter.iterator().next(), obj);

        final Iterable<Integer> iterNull = PSClassifyReadsUtils.wrapInIterable(null);
        Assert.assertTrue(iterNull.iterator().hasNext());
        Assert.assertNull(iterNull.iterator().next());
    }
    ...
    public void testDoHeaderWarnings() {
        ...
        PSClassifyReadsUtils.doHeaderWarnings(file.getAbsolutePath(), header, taxDB, options, logger);
        try {
    ...
    public void testMapGroupedReadsToTax(final int readLength, final List<Integer> NM1, final List<Integer> NM2,
        ...
        final JavaRDD<Tuple2<Iterable<GATKRead>, PSHitInfo>> resultXA = PSClassifyReadsUtils.mapGroupedReadsToTax(pairsXA, MIN_COV, MIN_IDENT, refNameToTaxBroadcast);
        final PSHitInfo infoXA = resultXA.first()._2;
        ...
        final JavaRDD<Tuple2<Iterable<GATKRead>, PSHitInfo>> resultSA = PSClassifyReadsUtils.mapGroupedReadsToTax(pairsSA, MIN_COV, MIN_IDENT, refNameToTaxBroadcast);
        final PSHitInfo infoSA = resultSA.first()._2;
    ...
    public void testComputeTaxScores() {
        ...
        List<PSHitInfo> readTaxHits = new ArrayList<>(1);
        readTaxHits.add(new PSHitInfo(Arrays.asList("4"), 2));
        try {
            final Map<String, PSScoreInfo> result = PSClassifyReadsUtils.computeTaxScores(readTaxHits, tree);
        ...
        readTaxHits.clear();
        readTaxHits.add(new PSHitInfo(Arrays.asList("3"), 2));
        Map<String, PSScoreInfo> result = PSClassifyReadsUtils.computeTaxScores(readTaxHits, tree);
        Assert.assertEquals(result.size(), 3);
        Assert.assertEquals(result.get("1").score, result.get("2").score);
        Assert.assertEquals(result.get("2").score, result.get("3").score);
        Assert.assertEquals(result.get("1").score, PSClassifyReadsUtils.SCORE_NORMALIZATION_FACTOR * 2.0 / 100);
        Assert.assertEquals(result.get("1").score_normalized, 100.0);
        Assert.assertEquals(result.get("2").score_normalized, 100.0);
        Assert.assertEquals(result.get("3").score_normalized, 100.0);
        Assert.assertEquals(result.get("3").reads, 2);
        Assert.assertEquals(result.get("3").unambiguous, 2);
        Assert.assertEquals(result.get("3").ref_length, 100);
        ...
        readTaxHits.clear();
        readTaxHits.add(new PSHitInfo(Arrays.asList("4"), 2)); //Invalid hit, ref length 0
        readTaxHits.add(new PSHitInfo(Arrays.asList("3"), 2));
        readTaxHits.add(new PSHitInfo(Arrays.asList("3", "6"), 2));
        readTaxHits.add(new PSHitInfo(Arrays.asList("5"), 2));
        readTaxHits.add(new PSHitInfo(Arrays.asList("6"), 1));
        readTaxHits.add(new PSHitInfo(Arrays.asList("8"), 2)); //Invalid hit, not in tree
        result = PSClassifyReadsUtils.computeTaxScores(readTaxHits, tree);

        final double score3 = PSClassifyReadsUtils.SCORE_NORMALIZATION_FACTOR * (0.5 * 2.0 + 2.0) / 100;
        final double score5 = PSClassifyReadsUtils.SCORE_NORMALIZATION_FACTOR * 2.0 / 100;
        final double score6 = PSClassifyReadsUtils.SCORE_NORMALIZATION_FACTOR * (0.5 * 2.0 + 1.0) / 100;
        ...
        Assert.assertEquals(result.get("1").score_normalized, 100.0 * score1 / score1);
        Assert.assertEquals(result.get("2").score_normalized, 100.0 * score2 / score1);
        Assert.assertEquals(result.get("3").score_normalized, 100.0 * score3 / score1);
        Assert.assertEquals(result.get("4").score_normalized, 100.0 * score4 / score1);
        Assert.assertEquals(result.get("5").score_normalized, 100.0 * score5 / score1);
        Assert.assertEquals(result.get("6").score_normalized, 100.0 * score6 / score1);
        ...
        Assert.assertEquals(result.get("1").ref_length, 0);
        Assert.assertEquals(result.get("3").ref_length, 100);
        Assert.assertEquals(result.get("4").ref_length, 0);
    ...
    public void testCollectTupleSecond(final List<Integer> intsList) {
        ...
        //Test
        final Iterable<Integer> result = PSClassifyReadsUtils.collectTupleSecond(dataRDD);
    ...
    public void testFlattenTupleFirstIterable(final List<int[]> arrayList) {
        ...
        //Check that the result is equal to the original integer list, allowing for different order
        final List<Integer> resultList = new ArrayList<>(PSClassifyReadsUtils.flattenTupleFirstIterable(dataRDD).collect());
