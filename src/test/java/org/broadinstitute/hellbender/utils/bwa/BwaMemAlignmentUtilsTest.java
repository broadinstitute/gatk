...
import org.broadinstitute.hellbender.tools.spark.sv.discovery.AlignmentIntervalUnitTest;
...
public class BwaMemAlignmentUtilsTest extends BaseTest {
    @Test
    ...
    @Test(dataProvider = "applyAlignmentData")
    public void testApplyAlignment(final BwaMemAlignment bwaMemAlignment, final SimpleInterval expectedReferenceInterval, final Cigar expectedCigar,
                                   final boolean expectedIsPositiveStrand, @SuppressWarnings("unused") final int expectedStartOnContig_1BasedInclusive, @SuppressWarnings("unused") final int expectedEndOnContig_1BasedInclusive,
                                   final int expectedContigLength, final int expectedMapQualInBwaMemAlignment, @SuppressWarnings("unused") final AlignmentInterval expectedAlignmentInterval) {
        final List<String> refNames = hg19Header.getSequenceDictionary().getSequences().stream().map(SAMSequenceRecord::getSequenceName).collect(Collectors.toList());
        ...
        final String errorListString = errors == null ? "" : samRecord.isValid().stream().filter(Objects::nonNull).map(SAMValidationError::getMessage).collect(Collectors.joining(", "));
        Assert.assertNull(errors, "sam record is not valid: " + errorListString);
        Assert.assertEquals(samRecord.getReadNegativeStrandFlag(), !expectedIsPositiveStrand);
        Assert.assertEquals(samRecord.getCigar(), expectedIsPositiveStrand ? expectedCigar : CigarUtils.invertCigar(expectedCigar));
        Assert.assertEquals(new SimpleInterval(samRecord.getContig(), samRecord.getStart(), samRecord.getEnd()), expectedReferenceInterval);
        Assert.assertEquals(samRecord.getMappingQuality(), Math.max(SAMRecord.NO_MAPPING_QUALITY,expectedMapQualInBwaMemAlignment));
    }

    @DataProvider(name = "applyAlignmentData")
    public Object[][] applyAlignmentData() {
        return AlignmentIntervalUnitTest.createInputsAndExpectedResults_BwaMemAlignmentConstruction();
    }
}
