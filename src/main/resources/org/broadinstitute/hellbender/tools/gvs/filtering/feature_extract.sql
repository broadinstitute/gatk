WITH
    ...
    aa_ref_ad_info AS (
        ...
        FROM (
               ...
               FROM `@altAllele`
               WHERE sample_id IN (SELECT sample_id FROM `@sample` WHERE is_loaded is TRUE AND (withdrawn IS NULL OR @withdrawnCutoffCondition))
               @locationStanza
   ...
   hq_genotype_qc AS (
    ...
    WHERE allele != '*'
    AND sample_id IN (SELECT sample_id FROM `@sample` WHERE is_loaded is TRUE AND (withdrawn IS NULL OR @withdrawnCutoffCondition))
    @locationStanza
    GROUP BY 1,2,3
    ) ai
