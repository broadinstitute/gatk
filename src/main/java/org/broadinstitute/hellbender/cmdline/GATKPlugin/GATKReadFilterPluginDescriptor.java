...
public class GATKReadFilterPluginDescriptor extends CommandLinePluginDescriptor<ReadFilter> {
    ...
    // Map of read filterSuffix (simple) class names to the corresponding discovered plugin instance
    private final Map<String, ReadFilter> allDiscoveredReadFilters = new HashMap<>();

    // Map of read filterSuffix (simple) class names to the corresponding default plugin instance
    ...
    public String getDisplayName() {
        ...
        // here because the kebabified version ("read-filterSuffix") isn't a valid freemarker variable (and is
    ...
    // Return a list with a read filterSuffix instance for each read filterSuffix enabled by the user.
    ...
    /**
     * Return the allowed values for read-filterSuffix/disable-read-filterSuffix names for use by the help system.
    ...
    public void validateAndResolvePlugins() {
        // throw if a filterSuffix is *enabled* more than once by the user
        ...
        if (!duplicateUserEnabledFilterNames.isEmpty()) {
            throw new CommandLineException.BadArgumentValue(
                    String.format("The read filterSuffix(s) are enabled more than once: %s",
                            Utils.join(", ", duplicateUserEnabledFilterNames)));
        }

        // throw if a filterSuffix is *disabled* more than once by the user
        ...
        if (!duplicateDisabledUserFilterNames.isEmpty()) {
            throw new CommandLineException.BadArgumentValue(
                    String.format("The read filterSuffix(s) are disabled more than once: %s",
                            Utils.join(", ", duplicateDisabledUserFilterNames)));
        }

        // throw if a filterSuffix is both enabled *and* disabled by the user
        ...
        if (!enabledAndDisabled.isEmpty()) {
            ...
            throw new CommandLineException(
                    String.format("The read filterSuffix(s): %s are both enabled and disabled", badFiltersList));
        }

        // throw if a disabled filterSuffix doesn't exist; warn if it wasn't enabled by the tool in the first place
        userArgs.getUserDisabledReadFilterNames().forEach(s -> {
            if (!allDiscoveredReadFilters.containsKey(s)) {
                throw new CommandLineException.BadArgumentValue(String.format("Disabled filterSuffix (%s) does not exist", s));
            } else if (!toolDefaultReadFilters.containsKey(s)) {
                logger.warn(String.format("Disabled filterSuffix (%s) is not enabled by this tool", s));
            }
        });

        // warn if a filterSuffix is both default and enabled by the user
        ...
        redundant.forEach(
            s -> {
                logger.warn(String.format("Redundant enabled filterSuffix (%s) is enabled for this tool by default", s));
            });

        // Throw if args were specified for a filterSuffix that was also disabled, or that was not enabled by the
        // tool by default.
        //
        ...
        // for that dependent argument has been supplied, either by a default read filterSuffix, or by an explicitly
        // enabled read filterSuffix. However, its possible for the user to subsequently try to disable that
        ...
        // throw if a filterSuffix name was specified that has no corresponding instance
        final Map<String, ReadFilter> requestedReadFilters = new HashMap<>();
        userArgs.getUserEnabledReadFilterNames().forEach(s -> {
            ReadFilter trf = allDiscoveredReadFilters.get(s);
            if (null == trf) {
                if (!toolDefaultReadFilters.containsKey(s)) {
                    throw new CommandLineException("Unrecognized read filterSuffix name: " + s);
                }
            } else {
     ...
     * tool defaults.
     * @param filterName name of the filterSuffix to query.
    ...
    /**
     * Merge the default filters with the users's command line read filterSuffix requests, then initialize
     * the resulting filters.
     *
     * @param samHeader - a SAMFileHeader to use to initialize read filterSuffix instances
     * @return Single merged read filterSuffix.
     */
     ...
     *
     * @param samHeader - a SAMFileHeader to use to initialize read filterSuffix instances
     * @return Single merged counting read filterSuffix.
     */
     ...
     * the resulting filters.
     *
     * @param samHeader a SAMFileHeader to initialize read filterSuffix instances. May not be null.
     * @param aggregateFunction function to use to merge ReadFilters, usually ReadFilter::fromList. The function
     *                          must return the ALLOW_ALL_READS filterSuffix wrapped in the appropriate type when passed
