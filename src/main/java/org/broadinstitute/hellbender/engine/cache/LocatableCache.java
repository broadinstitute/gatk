 ...
 *  end before the start of the new interval) via {@link # trimCache(SimpleInterval)}, then retrieve
 *  records up to the desired endpoint using {@link #getCachedLocatables(SimpleInterval)}.
 *
 * -If it is a cache miss, reset the cache using {@link #refillQueryCache(org.broadinstitute.hellbender.utils.SimpleInterval)},
...
public class LocatableCache<T extends Locatable> {
     ...
     */
    private void fill(final Iterator<T> locatableIter, final SimpleInterval interval ) {
        cache.clear();
     ...
     */
    private void refillQueryCache( final SimpleInterval queryInterval)
    {
        ...
        final SimpleInterval cacheInterval = cachingStrategy.getCacheIntervalFromQueryInterval(queryInterval);
     ...
     */
    @VisibleForTesting
    boolean cacheHit( final SimpleInterval interval ) {
     ...
     */
    @VisibleForTesting
    List<T> getCachedLocatables(final SimpleInterval interval ) {
