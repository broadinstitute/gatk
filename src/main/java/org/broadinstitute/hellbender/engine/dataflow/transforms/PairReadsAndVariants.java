...
/**
 * PairReadsAndVariants takes two PCollections (GATKRead and Variant) and create a single PCollection of KV where
 * GATKRead is the key and Variant is the value. We do this join by sharding both collections by "variant shard" and
 * checking for overlap on each "shard."
 * This currently will not work with unmapped reads.
 *
...
public class PairReadsAndVariants {
    static public PCollection<KV<GATKRead, Variant>> pair(PCollection<GATKRead> pRead, PCollection<Variant> pVariant) {
        ...
        PCollection<KV<VariantShard, GATKRead>> vkReads = pRead.apply(new KeyReadByVariantShard());
