...
import org.broadinstitute.hellbender.engine.dataflow.transforms.KeyReadByRefShard;
 ...
 *
 * step 2: group reads by shard (by start position)
 *
 *  |--------- shard 0 ----------|
 *            |------ read a -----|
 *
...
public class RefBasesForReads {
    public static PCollection<KV<GATKRead, ReferenceBases>> addBases(PCollection<GATKRead> pReads, RefAPISource refAPISource, RefAPIMetadata refAPIMetadata) {
        PCollection<KV<ReferenceShard, Iterable<GATKRead>>> shardAndRead = pReads.apply(new KeyReadByRefShard());
        PCollection<KV<ReferenceBases, Iterable<GATKRead>>> groupedReads =
                RefBasesFromAPI.getBasesForShard(shardAndRead, refAPISource, refAPIMetadata);
