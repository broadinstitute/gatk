 ...
 */
public final class ReferenceDataSourceFromReferenceBases implements ReferenceDataSource {

    private ReferenceBases bases;
    private SAMSequenceDictionary sequenceDictionary;

    /**
     * Initialize this data source using ReferenceBases and corresponding sequence dictionary.
     */
    public ReferenceDataSourceFromReferenceBases(final ReferenceBases bases, final SAMSequenceDictionary referenceSequenceDictionary) {
    ...
    public Iterator<Byte> query( final SimpleInterval interval ) {
        ...
        int startIndex = (bases.getInterval().getStart() - interval.getStart());
        // compute how many bases we're reading (+1 because those are closed intervals),
        // then add however many bases we're skipping at the front.
        int stopIndex = (interval.getStart() - interval.getEnd() + 1) + startIndex;
    ...
    public ReferenceSequence queryAndPrefetch( final String contig, final long start , final long stop) {
        ...
        int startIndex = (int)(bases.getInterval().getStart() - start);
        ...
        System.out.println("copyOfRange "+startIndex+", "+(startIndex+length));
        if (startIndex<0) {
