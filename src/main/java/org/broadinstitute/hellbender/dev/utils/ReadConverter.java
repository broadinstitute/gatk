...
/**
 * Converts a Genomics Read object to a SAMRecord object.
 *
 * Reads are used in the Dataflow pipelines. SAMRecords are used by existing GATK code.
 * In order to fit the existing GATK code into the dataflow pipelines, we have to convert.
 */
public class ReadConverter extends SAMRecord {

    /**
     * Converts a genomics.model.Read to a htsjdk.samtools.SAMRecord,
     * given the header.
     */
    public static SAMRecord toSAMRecord(Read read, SAMFileHeader header) {
        return new ReadConverter(read, header);
    }

    /**
     * Converts a genomics.model.CigarUnit to a htsjdk.samtools.CigarElement
     */
    public static htsjdk.samtools.CigarElement toSAMCigar(CigarUnit cu) {
    ...
    /**
     * Converts a htsjdk.samtools.CigarElement to a genomics.model.CigarUnit
     */
    public static CigarUnit toGenomicsCigar(htsjdk.samtools.CigarElement sam) {
    ...
    private ReadConverter(Read read, SAMFileHeader header) {
        super(header);
        ...
        if (null != alignment) {
            ...
            if (null != cigar) {
                this.setCigar(
                        new htsjdk.samtools.Cigar(read.getAlignment().getCigar().stream().map(ReadConverter::toSAMCigar).collect(Collectors.toList())));
            }
        }
        ...
        if (null != info) {
            for (Map.Entry<String, List<String>> entry : info.entrySet()) {
                ...
                try {
                    ...
                    // X,Y, or Z tags don't have an equivalent in SAMRecord, so we can't convert them.
                }
            }
        }
