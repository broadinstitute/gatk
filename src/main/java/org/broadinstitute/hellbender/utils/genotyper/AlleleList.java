...
public interface AlleleList<A extends Allele> {
    ...
    /**
     * Resolves the index of the reference allele in this allele-list.
     *
     * <p>
     ...
     *
     *
     * @throws IllegalArgumentException if the list has more than one reference.
     *
    ...
    default A getReferenceAllele() {
        final int index = indexOfReference();
        return index == -1 ? null : getAllele(index);
    }

    default A getUniqueReferenceAllele() {
        final int index = indexOfUniqueReference();
        return index == -1 ? null : getAllele(index);
    }

    /**
     * Check whether this and another list have the same alleles in the same order.
     */
        ...
        if (other == this) {
            ...
            if (thisLength == other.numberOfAlleles()) {
                for (int i = 0; i < thisLength; i++) {
                    if (!getAllele(i).equals(other.getAllele(i))) {
                        return false;
                    }
                }
                return true;
    ...
    /**
     * Resolves the index of the reference allele in this allele-list. If there is more than on reference
     * allele it throws an exception.
     *
     * @throws IllegalArgumentException if the list has more than one reference.
     *
     * @return -1 if there is no reference allele, or a values in [0,{@code list.alleleCount()}).
     */
    default int indexOfUniqueReference() {
        final int alleleCount = this.numberOfAlleles();
        for (int i = 0; i < alleleCount; i++) {
            if (getAllele(i).isReference()) {
                for (int j = i + 1; j < alleleCount; j++) {
                    if (getAllele(j).isReference()) {
                        throw new IllegalArgumentException("no more than one reference allowed");
                    }
                }
                return i;
            }
