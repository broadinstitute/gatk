...
public final class STRTableFileBuilder implements AutoCloseable {
    ...
    private STRTableFileBuilder(final File dir, final boolean generateTextSitesFile, final SAMSequenceDictionary dictionary,
        ...
        try {
            sitesWriter = DragstrLocus.binaryWriter(new File(dir, STRTableFile.SITES_FILE_NAME), new File(dir, STRTableFile.SITES_INDEX_FILE_NAME));
        } catch (final FileNotFoundException ex) {
            throw new GATKException("possible bug, the parent directory " + dir + " must exists at this point", ex);
        }
        try {
            textSitesWriter = generateTextSitesFile ? DragstrLocus.textWriter(new FileOutputStream(new File(dir, STRTableFile.SITES_TEXT_FILE_NAME)), dictionary) : null;
        } catch (IOException e) {
