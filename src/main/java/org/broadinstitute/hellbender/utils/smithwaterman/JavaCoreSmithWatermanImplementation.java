...
/**
 * Java implementation of the core SmithWaterman algorithm.
 */
public final class JavaCoreSmithWatermanImplementation implements CoreSmithWatermanAlgorithm {
    private SmithWatermanParameters parameters;
    ...
    */
    ...
    public boolean initialize(final SmithWatermanParameters params) {
        ...
        return true;//Java accepts everything
    }

    @Override
    public Cigar getCigar() {
        return alignmentResult.cigar;
    }

    @Override
    public int getAlignmentStart2wrt1() {
        return alignmentResult.alignment_offset;
    }
    ...
    private void calculateMatrix(final byte[] reference, final byte[] alternate, final int[][] sw, final int[][] btrack) {
        calculateMatrix(reference, alternate, sw, btrack, parameters.overhangStrategy);
    }
    ...
    public void align(final byte[] reference, final byte[] alternate) {
        ...
        int matchIndex = -1;
        if (parameters.overhangStrategy == OverhangStrategy.SOFTCLIP || parameters.overhangStrategy == OverhangStrategy.IGNORE) {
        ...
        if (matchIndex != -1) {
            ...
            alignmentResult = calculateCigar(sw, btrack, parameters.overhangStrategy); // length of the segment (continuous matches, insertions or deletions)
        }
    }
