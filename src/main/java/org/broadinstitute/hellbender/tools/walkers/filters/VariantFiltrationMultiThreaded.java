...
@CommandLineProgramProperties(
        summary = "EXPERIMENTAL multi-threaded variant of VariantFiltration. " +
                  "Filters variant calls based on INFO and/or FORMAT annotations using a worker thread " +
                  "pool to process variants in parallel while preserving input order in the output. " +
                  "See VariantFiltration for full documentation of filter criteria. " +
                  "Note: --mask and the SNP-clustering flags (--cluster-size + --cluster-window-size) " +
                  "are not safe with multiple threads in this experimental version because they query " +
                  "the shared FeatureManager; set --threads 1 if you need them.",
        oneLineSummary = "(Experimental) Multi-threaded VariantFiltration",
        programGroup = VariantFilteringProgramGroup.class
)
@DocumentedFeature
@ExperimentalFeature
public final class VariantFiltrationMultiThreaded extends MultiThreadedVariantWalker {
    ...
    public void onTraversalStart() {
        // Guard: these features touch shared mutable state (FeatureManager queries for the mask
        // track and for the cluster-window neighbor lookup) that is not safe for concurrent
        // workers. Refuse to run them with --threads > 1.
        if (numThreads != 1) {
            final List<String> unsafe = new ArrayList<>();
            if (mask != null) unsafe.add("--mask");
            if (clusterWindow > 0) unsafe.add("--" + CLUSTER_WINDOW_SIZE_LONG_NAME);
            if (!unsafe.isEmpty()) {
                throw new org.broadinstitute.hellbender.exceptions.UserException.BadInput(
                        "VariantFiltrationMultiThreaded does not yet support " + String.join(", ", unsafe)
                        + " when running with multiple worker threads. Re-run with --" +
                        THREADS_LONG_NAME + " 1, or use the standard VariantFiltration tool.");
            }
        }
        if (clusterSize <= 1){
    ...
    /**
     * Per-variant work: build the filtered output VC (either by splitting multi-allelics and filtering
     * each, or by filtering the variant directly). Pure with respect to mutable instance state when
     * the unsafe-feature guards in {@link #onTraversalStart} have passed (no {@code --mask} and no SNP
     * clustering with multiple threads).
     */
    @Override
    protected List<VariantContext> applyParallel(final VariantContext variant,
                                                 final ReadsContext readsContext,
                                                 final ReferenceContext ref,
                                                 final FeatureContext featureContext) {
        final VariantContext out;
        if (applyForAllele) {
            final List<VariantContext> filtered = splitMultiAllelics(variant).stream()
                    .map(vc -> filter(vc, new FeatureContext(featureContext, new SimpleInterval(vc.getContig(), vc.getStart(), vc.getEnd()))))
                    .collect(Collectors.toList());
            final List<Set<String>> alleleFilters = filtered.stream().map(VariantContext::getFilters).collect(Collectors.toList());
            out = AlleleFilterUtils.addAlleleAndSiteFilters(variant, alleleFilters, invalidatePreviousFilters);
        } else {
            out = filter(variant, featureContext);
        }
        return java.util.Collections.singletonList(out);
    }

    /**
     * Drainer-thread output: writes the produced VC (always exactly one per input variant; this tool
     * never drops variants — it only tags them with FILTER strings).
     */
    @Override
    protected void consumeOutput(final VariantContext inputVariant, final List<VariantContext> outputs) {
        for (final VariantContext vc : outputs) {
            writer.add(vc);
        }
    }
