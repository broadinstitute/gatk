...
public final class ReferenceConfidenceModel {
    ...
    public ReferenceConfidenceModel(final GenomeLocParser genomeLocParser,
        ...
        Utils.nonNull( header == null, "header cannot be empty");
    ...
    public List<VariantContext> calculateRefConfidence(final Haplotype refHaplotype,
        ...
        Utils.nonNull(refHaplotype == null, "refHaplotype cannot be null");
        Utils.nonNull(calledHaplotypes == null, "calledHaplotypes cannot be null");
        ...
        Utils.nonNull(paddedReferenceLoc == null, "paddedReferenceLoc cannot be null");
        Utils.nonNull(activeRegion == null, "activeRegion cannot be null");
        Utils.nonNull(readLikelihoods == null, "readLikelihoods cannot be null");
        ...
        Utils.nonNull(ploidyModel == null, "the ploidy model cannot be null");
        Utils.nonNull(model == null, "the genotyping model cannot be null");
        ...
        for ( final ReadPileup pileup : refPileups ) {
            ...
            if ( overlappingSite != null && overlappingSite.getStart() == curPos.getStart() ) {
                ...
                gb.AD(homRefCalc.get_AD_Ref_Any());
                gb.DP(homRefCalc.getDP());

                // genotype likelihood calculation
                final GenotypeLikelihoods snpGLs = GenotypeLikelihoods.fromLog10Likelihoods(homRefCalc.getGenotypeLikelihoods());
                ...
                gb.GQ((int) (-10 * GenotypeLikelihoods.getGQLog10FromLikelihoods(GenotypeType.HOM_REF.ordinal()-1, leastConfidenceGLs.getAsVector())));
    ...
    GenotypeLikelihoods getGLwithWorstGQ(final GenotypeLikelihoods gl1, final GenotypeLikelihoods gl2) {
        return GenotypeLikelihoods.getGQLog10FromLikelihoods(GenotypeType.HOM_REF.ordinal()-1, gl1.getAsVector())
                >
               GenotypeLikelihoods.getGQLog10FromLikelihoods(GenotypeType.HOM_REF.ordinal()-1, gl2.getAsVector()) ? gl1 : gl2;
    }
    ...
    private RefVsAnyResult calcGenotypeLikelihoodsOfRefVsAny(final int ploidy,
        ...
        for (int i = 0; i < likelihoodCount; i++) {
            result.increaseGenotypeLikelihood(i, -denominator);
        }
        return result;
    }

    private void calcPileupElementRefVsNonRefLikelihoodAndCount(final byte refBase, final int likelihoodCount, final double log10Ploidy, final RefVsAnyResult result, final PileupElement element, final byte qual, final Mean hqSoftClips) {
        ...
        result.increaseGenotypeLikelihood(0, referenceLikelihood + log10Ploidy);
        result.increaseGenotypeLikelihood(likelihoodCount - 1, nonRefLikelihood + log10Ploidy);
        // Heterozygous likelihoods need the logSum trick:
        for (int i = 1, j = likelihoodCount - 2; i < likelihoodCount - 1; i++, j--) {
            result.increaseGenotypeLikelihood(i,
    ...
    private List<ReadPileup> getPileupsOverReference(final Haplotype refHaplotype,
        ...
        if ( debuggingWriter != null ) {
            for (final GATKRead read : reads) {
                debuggingWriter.addAlignment(read.convertToSAMRecord(activeRegion.getHeader()));
            }
        }

        final LocusIteratorByState libs = new LocusIteratorByState(reads.iterator(), LocusIteratorByState.NO_DOWNSAMPLING,
                true, genomeLocParser, samples.asSetOfSamples(), false, activeRegion.getHeader());
