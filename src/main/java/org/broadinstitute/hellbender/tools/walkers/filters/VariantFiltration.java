...
public final class VariantFiltration extends VariantWalker {
    ...
    private void initializeVcfWriter() {
        ...
        if (applyForAllele) {
            hInfo.add(new VCFInfoHeaderLine(AS_FILTER_STATUS_KEY, 1, VCFHeaderLineType.String, "Filter status for each allele, as assessed by ApplyRecalibration. Note that the VCF filter field will reflect the most lenient/sensitive status across all alleles."));
        }
