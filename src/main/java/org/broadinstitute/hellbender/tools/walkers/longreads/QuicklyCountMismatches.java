...
public final class QuicklyCountMismatches extends LocusWalker {
    ...
    private Map<Integer, Long> numMismatchesMap = new TreeMap<>();
    private Map<Integer, Long> numInsertionsMap = new TreeMap<>();
    private Map<Integer, Long> numDeletionsMap = new TreeMap<>();
    private Map<Integer, Long> numBasesMap = new TreeMap<>();
    private long numMismatches = 0;
    private long numBases = 0;

    private Map<Integer, Integer> insHist = new TreeMap<>();
    private Map<Integer, Integer> delHist = new TreeMap<>();

    @Override
    public boolean includeDeletions() { return true; }
    ...
    public void closeTool() {
        try {
            ...
            PrintStream outIns = new PrintStream(insPathName);
            for (int len : insHist.keySet()) {
                outIns.println(Joiner.on(" ").join(len, insHist.get(len)));
            }

            PrintStream outDel = new PrintStream(delPathName);
        ...
        //logger.info("numBases: " + numBases + " numMismatches:" + numMismatches);
    }
}
