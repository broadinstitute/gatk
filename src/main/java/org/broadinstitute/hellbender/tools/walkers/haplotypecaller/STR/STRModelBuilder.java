...
import org.broadinstitute.hellbender.utils.commandline.CommandLineProgram;
...
public final class STRModelBuilder extends CommandLineProgram {
    ...
    @Argument(doc = "count output directory", fullName = "countOutputDirectory", shortName = "countDir", required = false)
    protected File countDir;

    private List<Pair<STRContext, Map<String, List<STRAllele>>>> trainingData;
    ...
    public void addTrainingData(final STRContext context, final Map<String, List<STRAllele>> truth) {
        if (validLikelihoods(context)) {
            trainingData.add(new Pair<>(context, truth));
        } else {
    ...
    private boolean validLikelihoods(final STRContext context) {
        ...
        for (final String sample : likelihoods.samples()) {
            final ReadLikelihoods.Matrix<STRAllele> sampleMatrix = likelihoods.sampleMatrix(sample);
            final int readCount = sampleMatrix.readCount();
    ...
    public STRModelParameterCollection fitTrainingData(final STRModelCovariate.List covariates) {
        ...
        final Map<Integer, List<Pair<STRContext, Map<String, List<STRAllele>>>>> trainingDataBySetIndex =
                trainingData.stream().collect(Collectors.groupingBy(pair -> covariates.indexFor(pair.getFirst())));
        final List<STRModelParameters> sets = new ArrayList<>(setCount);
        logger.info("Training elements: " + setCount);
        for (int i = 0; i < setCount; i++) {
            final List<Pair<STRContext, Map<String, List<STRAllele>>>> setData = trainingDataBySetIndex.getOrDefault(i, Collections.emptyList());
            if (setData.isEmpty()) {
    ...
    private void writeCounts(final STRModelCovariate.List covariates, final int index, final List<Pair<STRContext, Map<String, List<STRAllele>>>> setData) {
        countDir.mkdir();
        ...
        try (final PrintWriter writer = new PrintWriter(new FileWriter(fileName))) {
            ...
            for (final Pair<STRContext, Map<String, List<STRAllele>>> pair : setData) {
                ...
                final STRContext context = pair.getFirst();
                final int truth = context.getAlleles().indexOf(pair.getSecond().values().stream().findFirst().get().get(0));
    ...
    private STRModelParameters fitTrainingData(final STRModelCovariate.List covariates, final List<Pair<STRContext, Map<String, List<STRAllele>>>> setData) {
    ...
    private double calculateErrorRateDerivative(final List<Pair<STRContext, Map<String, List<STRAllele>>>> setData, final STRModelParameters result) {
        return calculateDerivative(setData, result::piDerivativeMatrix, result::log10TransformationMatrix);
    }

    private double calculateLambdaDerivative(final List<Pair<STRContext, Map<String, List<STRAllele>>>> setData, final STRModelParameters result) {
        return calculateDerivative(setData, result::lambdaDerivativeMatrix, result::log10TransformationMatrix);
    }

    private double calculateLog10Likelihood(final List<Pair<STRContext, Map<String, List<STRAllele>>>> setData,
        ...
        for (final Pair<STRContext, Map<String, List<STRAllele>>> setDatum : setData) {
            final ReadLikelihoods<STRAllele> likelihoods = setDatum.getFirst().getLikelihoods();
            ...
            final int likelhoodAlleleCount = likelihoods.alleleCount();
            ...
            int nextReadIndex = 0;
            for (final String sample : likelihoods.samples()) {
                final List<STRAllele> truth = setDatum.getSecond().get(sample);
                final int truthAlleleCount = truth.size();
                final double[] truthAlleleLikelihoods = new double[truthAlleleCount];
                final ReadLikelihoods.Matrix<STRAllele> sampleLikelihoods = likelihoods.sampleMatrix(likelihoods.sampleIndex(sample));
                ...
                for (int readIndex = 0; readIndex < readCount; readIndex++) {
                    for (int truthAlleleIndex = 0; truthAlleleIndex < truthAlleleCount; truthAlleleIndex++) {
                        final int truthAlleleLikelihoodIndex = likelihoods.alleleIndex(truth.get(truthAlleleIndex));
                        ...
                        final double log10TruthAlleleReadLikelihood = MathUtils.log10sumLog10(likelihoodAlleleContributions) - MathUtils.Log10Cache.get(truthAlleleCount);
    ...
    private double calculateDerivative(final List<Pair<STRContext, Map<String, List<STRAllele>>>> setData,
            ...
            for (final String sample : likelihoods.samples()) {
                ...
                for (int readIndex = 0; readIndex < readCount; readIndex++) {
                    for (int truthAlleleIndex = 0; truthAlleleIndex < truthAlleleCount; truthAlleleIndex++) {
                        ...
                        truthAlleleLikelihood[truthAlleleIndex] = MathUtils.log10sumLog10(likelihoodAlleleContributions) - MathUtils.Log10Cache.get(truthAlleleCount);
    ...
    private double initialLambda(final List<Pair<STRContext, Map<String, List<STRAllele>>>> setData) {
        double totalSum = 1.0;
        double totalWeight = 1.0;
        for (final Pair<STRContext, Map<String, List<STRAllele>>> setDatum : setData) {
            final ReadLikelihoods<STRAllele> likelihoods = setDatum.getFirst().getLikelihoods();
            for (final String sample : setDatum.getSecond().keySet()) {
                final List<STRAllele> truth = setDatum.getSecond().get(sample);
                final OptionalDouble averageDistance = likelihoods.sampleMatrix(sample).bestAlleles().stream()
                ...
                if (averageDistance.isPresent()) {
                    final double weight = Math.log(likelihoods.sampleMatrix(sample).readCount()); // avoid giving to much weight to sites with many reads.
                    totalWeight += weight;
    ...
    private double estimateErrorRate(final List<Pair<STRContext, Map<String, List<STRAllele>>>> setData) {
        ...
        for (final Pair<STRContext, Map<String, List<STRAllele>>> setDatum : setData) {
            ...
            for (final String sample : setDatum.getSecond().keySet()) {
                ...
                final double weight = Math.log(likelihoods.sampleMatrix(sample).readCount()); // avoid giving to much weight to sites with many reads.
                totalWeight += weight;
                final long mismatches = likelihoods.bestAlleles().stream().filter(ba -> !truth.contains(ba.allele)).count();
    ...
    private GATKSAMRecord readFactory(final String name) {
...
//                }
    ...
    public static void main(String[] args) {
        try {
            final STRModelBuilder instance = new STRModelBuilder();
            start(instance, args);
