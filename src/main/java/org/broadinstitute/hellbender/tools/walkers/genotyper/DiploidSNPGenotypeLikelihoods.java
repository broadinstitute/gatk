...
public class DiploidSNPGenotypeLikelihoods implements Cloneable {
     ...
     */
    public int add(ReadPileup pileup, boolean ignoreBadBases, boolean capBaseQualsAtMappingQual, int minBaseQual) {
        int n = 0;

        // for each fragment, add to the likelihoods
        FragmentCollection<PileupElement> fpile = FragmentCollection.create(pileup);
    ...
    private static ThreadLocal<DiploidSNPGenotypeLikelihoods[][][][][]> CACHE = new ThreadLocal<DiploidSNPGenotypeLikelihoods[][][][][]>() {
        @Override
        protected DiploidSNPGenotypeLikelihoods[][][][][] initialValue() {
            return new DiploidSNPGenotypeLikelihoods[BaseUtils.BASES.length][QualityUtils.MAX_SAM_QUAL_SCORE +1][BaseUtils.BASES.length+1][QualityUtils.MAX_SAM_QUAL_SCORE +1][MAX_PLOIDY];
        }
    };

    protected boolean inCache(byte observedBase1, byte qualityScore1, byte observedBase2, byte qualityScore2, int ploidy) {
        return getCache(CACHE.get(), observedBase1, qualityScore1, observedBase2, qualityScore2, ploidy) != null;
    }
    ...
    protected DiploidSNPGenotypeLikelihoods getCachedGenotypeLikelihoods(byte observedBase1, byte qualityScore1, byte observedBase2, byte qualityScore2, int ploidy) {
        DiploidSNPGenotypeLikelihoods gl = getCache(CACHE.get(), observedBase1, qualityScore1, observedBase2, qualityScore2, ploidy);
        if ( gl == null )
    ...
    protected DiploidSNPGenotypeLikelihoods calculateCachedGenotypeLikelihoods(byte observedBase1, byte qualityScore1, byte observedBase2, byte qualityScore2, int ploidy) {
        ...
        setCache(CACHE.get(), observedBase1, qualityScore1, observedBase2, qualityScore2, ploidy, gl);
        return gl;
    }
    ...
    private static byte qualToUse(PileupElement p, boolean ignoreBadBases, boolean capBaseQualsAtMappingQual, int minBaseQual) {
        ...
        if ( qual > SAMUtils.MAX_PHRED_SCORE )
            throw new UserException.MalformedRead(p.getRead(), "we encountered an extremely high quality score (" + (int)qual + ")");
    ...
    public boolean validate(boolean throwException) {
        try {
            for ( DiploidGenotype g : DiploidGenotype.values() ) {
                ...
                int i = g.ordinal();
                if ( ! MathUtils.wellFormedDouble(log10Likelihoods[i]) || log10Likelihoods[i] > 0.0 ) {
