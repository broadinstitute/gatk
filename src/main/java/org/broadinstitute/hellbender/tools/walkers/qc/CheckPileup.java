...
@CommandLineProgramProperties(
    summary = "At every locus in the input set, compares the pileup data (reference base, aligned base from each overlapping"
        + "read, and quality score) generated internally by GATK to a reference pileup data generated by Samtools. Note "
        + "that the pileup program has been replaced in Samtools by mpileup, which produces a slightly different output "
        + "format by default.",
    oneLineSummary = "Compare GATK's internal pileup to a reference Samtools pileup",
    programGroup = QCProgramGroup.class)
public class CheckPileup extends LocusWalker {
    ...
    FeatureInput<SAMPileupFeature> pileup;
     ...
     */
    private SAMPileupFeature getTruePileup( FeatureContext tracker ) {
        final List<SAMPileupFeature> features = tracker.getValues(pileup);
        SAMPileupFeature pileupArg = (features.isEmpty()) ? null : features.get(0);
        ...
        if( pileupArg.hasPointGenotype() ) {
            ...
            throw new GATKException("Unsupported pileup type: " + pileupArg);
        }
    }
