 ...
 */
public class HaplotypeCallerEngine {
    ...
    private void initializeActiveRegionEvaluationGenotyperEngine() {
        ...
        final UnifiedArgumentCollection simpleUAC = hcArgs.cloneTo(UnifiedArgumentCollection.class);
    ...
    public static CountingReadFilter makeStandardHCReadFilter( final HaplotypeCallerArgumentCollection hcArgs, final SAMFileHeader header ) {
        return new CountingReadFilter("MAPPING_QUALITY", new MappingQualityReadFilter(hcArgs.MIN_MAPPING_QUALITY_SCORE))
                ...
                .and(new CountingReadFilter("CONSUMES_REFERENCE_BASES", read -> read.getStart() <= read.getEnd()))  // TODO: different from GATK3
    ...
    public ActivityProfileState isActive( final FeatureContext features, final ReferenceContext ref, final AlignmentContext context ) {
        ...
        if (genotypes.size() == 1) {
            ...
            final VariantCallContext vcOut = activeRegionEvaluationGenotyperEngine.calculateGenotypes(new VariantContextBuilder("HCisActive!", context.getContig(), context.getLocation().getStart(), context.getLocation().getEnd(), alleles).genotypes(genotypes).make(), GenotypeLikelihoodsCalculationModel.Model.SNP, readsHeader);
     ...
     */
    protected AssemblyResultSet assembleReads(final AssemblyRegion region, final List<VariantContext> giveAlleles) {
