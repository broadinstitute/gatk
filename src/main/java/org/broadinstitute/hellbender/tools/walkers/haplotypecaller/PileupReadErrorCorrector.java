...
public class PileupReadErrorCorrector implements ReadErrorCorrector {
    ...
    public final List<GATKRead> correctReads(final Collection<GATKRead> originalRreads) {
        ...
        final Map<GATKRead, List<Pair<Integer, Byte>>> corrections = reads.stream().collect(Collectors.toMap(read -> read, read -> new ArrayList<>()));
            ...
            if (logOdds < logOddsThreshold) {
                for (final PileupElement pe : pileup) {
                    if (pe.getBase() != ref && !(pe.isDeletion() || pe.isBeforeInsertion() || pe.isAfterDeletionEnd() || pe.isBeforeDeletionStart() || pe.isAfterInsertion() || pe.isAfterSoftClip())) {
                        corrections.get(pe.getRead()).add(ImmutablePair.of(pe.getOffset(), ref));
                    }
                }
            }
        ...
        corrections.entrySet().forEach(entry -> {
            final GATKRead read = entry.getKey();
            final byte[] bases = read.getBasesNoCopy();
            final int length = bases.length;
            final byte[] quals = read.getBaseQualitiesNoCopy();
            final List<Pair<Integer, Byte>> correctionsList = entry.getValue();
            final int size = correctionsList.size();

            // if read starts with a cluster of corrections, see how far this possible indel extends and skip those leading corrections
            // otherwise, start from the first correction
            final int firstCorrectionToApply = size >= INDEL_MISMATCHES && correctionsList.get(INDEL_MISMATCHES - 1).getLeft() < INDEL_SPAN ?
                    IntStream.range(INDEL_MISMATCHES - 1, size)
                        .filter(n -> correctionsList.get(n).getLeft() - correctionsList.get(n - INDEL_MISMATCHES + 1).getLeft() > INDEL_SPAN)
                        .findFirst().orElse(size)
                    : 0;

            // if read starts with a cluster of corrections, see how far this possible indel extends and skip those leading corrections
            // otherwise, start from the first correction
            final int lastCorrectionToApply = size >= INDEL_MISMATCHES && correctionsList.get(size - INDEL_MISMATCHES).getLeft() > length - INDEL_SPAN ?
                    IntStream.range(0, size - INDEL_MISMATCHES).boxed().sorted(Comparator.reverseOrder())
                            .filter(n -> correctionsList.get(n + INDEL_MISMATCHES - 1).getLeft() - correctionsList.get(n).getLeft() > INDEL_SPAN)
                            .findFirst().orElse(-1)
                    : size - 1;


            for (int n = firstCorrectionToApply; n <= lastCorrectionToApply; n++) {
                bases[correctionsList.get(n).getLeft()] = correctionsList.get(n).getRight();
                quals[correctionsList.get(n).getLeft()] = GOOD_QUAL;
            }
