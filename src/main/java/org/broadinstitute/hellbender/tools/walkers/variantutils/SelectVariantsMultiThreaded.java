...
import org.broadinstitute.hellbender.engine.MultiThreadedVariantWalker;
...
@CommandLineProgramProperties(
        summary = "EXPERIMENTAL multi-threaded variant of SelectVariants. " +
                  "Selects a subset of variants from a VCF file using a worker thread pool to process " +
                  "variants in parallel while preserving input order in the output. See SelectVariants " +
                  "for full documentation of selection criteria. " +
                  "Note: --discordance, --concordance, and --mendelian-violation are not safe with " +
                  "multiple threads in this experimental version; set --threads 1 if you need them.",
        oneLineSummary = "(Experimental) Multi-threaded SelectVariants",
        programGroup = VariantManipulationProgramGroup.class
)
@DocumentedFeature
@ExperimentalFeature
public final class SelectVariantsMultiThreaded extends MultiThreadedVariantWalker {
    ...
    // Note: previous versions of this class held a per-tool monitor (htsjdkLock) around the
    // htsjdk VC read paths (subContextFromSamples, getGenotypes(samples), fullyDecode, ...) to
    // work around concurrent-read corruption in htsjdk's lazy caches. With the thread-safe
    // patches that ship in htsjdk 4.3.0-1-ge73d96c-SNAPSHOT and newer
    // (LazyGenotypesContext.decode, GenotypesContext.ensureSample*, VariantContext lazy caches
    // all use volatile + double-checked locking) that lock is no longer required and has been
    // removed.
    ...
    public void onTraversalStart() {
        // Guard: these features touch shared mutable state (FeatureManager queries, MendelianViolation
        // counter) that is not safe for concurrent workers. Refuse to run them with --threads > 1.
        if (numThreads != 1) {
            final List<String> unsafe = new ArrayList<>();
            if (discordanceTrack != null)         unsafe.add("--discordance");
            if (concordanceTrack != null)         unsafe.add("--concordance");
            if (mendelianViolations)              unsafe.add("--mendelian-violation");
            if (!unsafe.isEmpty()) {
                throw new UserException.BadInput(
                        "SelectVariantsMultiThreaded does not yet support " + String.join(", ", unsafe)
                        + " when running with multiple worker threads. Re-run with --" +
                        THREADS_LONG_NAME + " 1, or use the standard SelectVariants tool.");
            }
        }
    ...
    /**
     * Per-variant work: all filter checks, sample/allele subsetting, JEXL evaluation, and annotation
     * dropping. Pure with respect to mutable instance state when the unsafe-feature guards in
     * {@link #onTraversalStart} have passed (no {@code --discordance}/{@code --concordance}/{@code --mv}
     * with multiple threads). Returns an empty list when the variant is filtered out, or a one-element
     * list containing the transformed variant to emit.
     */
    @Override
    protected List<VariantContext> applyParallel(VariantContext vc,
                                                 ReadsContext readsContext,
                                                 ReferenceContext ref,
                                                 FeatureContext featureContext) {

        // htsjdk's VC read path is now thread-safe (see htsjdk feature/thread-safe-vcf-read):
        // LazyGenotypesContext.decode, GenotypesContext.ensureSample*, and VariantContext's
        // lazy caches (type, monomorphic, genotypeCounts, ...) all use volatile + DCL upstream.
        // Workers can run fullyDecode, getGenotypes(samples), subContextFromSamples concurrently
        // on the same VC instance without external synchronization.
        if (fullyDecode) {
        ...
        // Second-pass JEXL evaluation on the (possibly subsetted) result. Pure read; parallel-safe.
        if (!applyJexlFiltersBeforeFilteringGenotypes && !passesJexlFilters(result)) {
            return Collections.emptyList();
        }

        return Collections.singletonList(dropAnnotations(result, infoAnnotationsToDrop, genotypeAnnotationsToDrop));
    }

    /**
     * Drainer-thread output handling. Invoked in input-variant order. Drains buffered variants whose
     * position can no longer be displaced by allele trimming of the current input, then buffers the
     * worker's outputs. Buffering preserves emission order even when allele trimming shifts a variant's
     * start position to the right.
     */
    @Override
    protected void consumeOutput(final VariantContext inputVariant, final List<VariantContext> outputs) {
        // Drain pending variants whose start is at or before the current input's start, or whose contig
        // no longer matches. Allele trimming can only shift a variant's start to the right, so anything
        // still pending with start <= inputVariant.start is safe to emit before the new outputs.
        while (!pendingVariants.isEmpty() &&
                (pendingVariants.peek().getStart() <= inputVariant.getStart()
                 || !pendingVariants.peek().getContig().equals(inputVariant.getContig()))) {
            vcfWriter.add(pendingVariants.poll());
        }
        for (final VariantContext out : outputs) {
            pendingVariants.add(out);
        }
    }
    ...
    private boolean applyFirstRoundOfFiltering(final VariantContext vc, final FeatureContext featureContext){
        ...
        if (fractionToKeep < 1.0) {
            final double draw;
            // Utils.getRandomGenerator() returns a shared, seeded singleton. Synchronize across worker
            // threads to preserve thread-safety and the seed-determined sequence semantics.
            synchronized (Utils.getRandomGenerator()) {
                draw = Utils.getRandomGenerator().nextDouble();
            }
            if (draw >= fractionToKeep) {
                return true;
            }
        }
    ...
    private VariantContext subsetGenotypesBySampleNames(final VariantContext vc, final boolean preserveAlleles, final boolean removeUnusedAlternates) {
        ...
        // randomGenotypes (java.util.Random) is not thread-safe; synchronize when called from worker threads.
        if (fractionGenotypes > 0) {
            final List<Genotype> genotypes = newGC.stream().map(genotype -> nextRandomGenotypeDraw() > fractionGenotypes ? genotype :
    ...
    /**
     * Thread-safe draw from the per-tool {@link #randomGenotypes} {@link Random}. Java's {@link Random}
     * is not safe for concurrent use; serialize draws across worker threads.
     */
    private double nextRandomGenotypeDraw() {
        synchronized (randomGenotypes) {
            return randomGenotypes.nextDouble();
        }
    }
