...
public final class ExcessHet extends PedigreeAnnotation implements StandardAnnotation, ReducibleAnnotation {
    ...
    private static final int NUM_LIST_ENTRIES = 3;
    private static final int HOM_REF_INDEX = 0;
    private static final int HET_INDEX = 1;
    private static final int VAR_INDEX = 2;
     ...
     *
     * @param vcAlleles   The merged allele list across all variants being combined/merged
     * @param annotationList The raw data for all the variants being combined/merged
    ...
    private static List<Integer> parseRawDataString(String rawDataString) {
        try {
            ...
            final int hr = Integer.parseInt(parsed[HOM_REF_INDEX]);
            final int het = Integer.parseInt(parsed[HET_INDEX]);
            final int hv = Integer.parseInt(parsed[VAR_INDEX]);
            return Arrays.asList(hr, het, hv);
    ...
    private String makeRawAnnotationString(final List<Allele> vcAlleles, final Map<Allele, List<Long>> perAlleleData) {
        return String.format("%d,%d,%d", perAlleleData.get(Allele.NO_CALL).get(HOM_REF_INDEX),
                perAlleleData.get(Allele.NO_CALL).get(HET_INDEX),
                perAlleleData.get(Allele.NO_CALL).get(VAR_INDEX));
    }
    ...
    private void combineAttributeMap(ReducibleAnnotationData<List<Integer>> toAdd, ReducibleAnnotationData<List<Integer>> combined) {
        if (combined.getAttribute(Allele.NO_CALL) != null) {
            combined.putAttribute(Allele.NO_CALL, Arrays.asList(
                    combined.getAttribute(Allele.NO_CALL).get(HOM_REF_INDEX) + toAdd.getAttribute(Allele.NO_CALL).get(HOM_REF_INDEX),
                    combined.getAttribute(Allele.NO_CALL).get(HET_INDEX) + toAdd.getAttribute(Allele.NO_CALL).get(HET_INDEX),
                    combined.getAttribute(Allele.NO_CALL).get(VAR_INDEX) + toAdd.getAttribute(Allele.NO_CALL).get(VAR_INDEX)));
        } else {
