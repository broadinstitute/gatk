 ...
 *
*/
public final class NearbyKmerErrorCorrector implements ReadErrorCorrector {
     ...
     */
    public NearbyKmerErrorCorrector(final int kmerLength,
     ...
     */
    public NearbyKmerErrorCorrector(final int kmerLength, final byte minTailQuality, final int minObservationsForKmerToBeSolid, final boolean debug, final byte[] fullReferenceWithPadding) {
    ...
    */
    private void addReadKmers(final GATKRead read) {
        Utils.nonNull(read);
     ...
     */
    @VisibleForTesting
    void addReadsToKmers(final Collection<GATKRead> reads) {
        Utils.nonNull(reads);
        for (final GATKRead read: reads) {
            addReadKmers(read);
        }
