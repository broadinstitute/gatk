...
public class ScikitLearnVariantTrainDataManager {
    ...
    private final ScikitLearnVariantTrainArgumentCollection VTAC;
    protected final static Logger logger = LogManager.getLogger(ScikitLearnVariantTrainDataManager.class);
    ...
    public void normalizeData(final boolean calculateMeans, List<Integer> theOrder) {
        ...
        for( final VariantDatum datum : data ) {
            ...
            for( final double val : datum.annotations ) {
                remove = remove || (Math.abs(val) > VTAC.STD_THRESHOLD);
            }
    ...
    public List<VariantDatum> getTrainingData() {
        ...
        for( final VariantDatum datum : data ) {
            ...
            } else if (datum.failingSTDThreshold && VTAC.debugStdevThresholding) {
        ...
        if( trainingData.size() > VTAC.MAX_NUM_TRAINING_DATA ) {
            logger.warn( "WARNING: Very large training set detected. Downsampling to " + VTAC.MAX_NUM_TRAINING_DATA + " training variants." );
            Collections.shuffle(trainingData, Utils.getRandomGenerator());
            return trainingData.subList(0, VTAC.MAX_NUM_TRAINING_DATA);
        }
        return trainingData;
    }
    ...
    public void decodeAnnotations( final VariantDatum datum, final VariantContext vc, final boolean jitter ) {
        ...
        for( final String key : annotationKeys ) {
            isNull[iii] = false;
            annotations[iii] = decodeAnnotation( key, vc, jitter, VTAC, datum );
    ...
    private static double decodeAnnotation( final String annotationKey, final VariantContext vc, final boolean jitter, final ScikitLearnVariantTrainArgumentCollection vtac, final VariantDatum datum ) {
        double value;
        ...
        try {
            ...
            if(vtac.useASannotations && annotationKey.startsWith(GATKVCFConstants.ALLELE_SPECIFIC_PREFIX)) {
            ...
            if( jitter && (annotationKey.equalsIgnoreCase(VCFConstants.RMS_MAPPING_QUALITY_KEY))) {
                if( vtac.MQ_CAP > 0) {
                    value = logitTransform(value, -SAFETY_OFFSET, vtac.MQ_CAP + SAFETY_OFFSET);
                    if (MathUtils.compareDoubles(value, logitTransform(vtac.MQ_CAP, -SAFETY_OFFSET, vtac.MQ_CAP + SAFETY_OFFSET), PRECISION) == 0 ) {
                        value += vtac.MQ_JITTER * Utils.getRandomGenerator().nextGaussian();
                    }
                } else if( MathUtils.compareDoubles(value, vtac.MQ_CAP, PRECISION) == 0 ) {
                    value += vtac.MQ_JITTER * Utils.getRandomGenerator().nextGaussian();
                }
            }
    ...
    public void parseTrainingSets(
        ...
        for( final TrainingSet trainingSet : trainingSets ) {
            ...
            for( final VariantContext trainVC : vcs ) {
                if (VTAC.useASannotations && !doAllelesMatch(trainVC, datum))
                    continue;
