...
public class AlleleFrequency extends VariantStratifier {
    ...
    private static int LOGIT_BIN_WIDTH = 1;
    private static int LOGIT_LIMIT = 30; // go from -30 to 30 using logit function
    ...
    public void initialize() {
        ...
        switch (scale) {
            case LINEAR:
                ...
                break;
            case LOGARITHMIC:
                for (int a = -LOGIT_LIMIT; a <= LOGIT_LIMIT; a += LOGIT_BIN_WIDTH) {
            ...
            try {
                ...
                if (useCompAFStratifier) {
                    if (comp != null) {
                        ...
                        alleleFrequency = 0.0; // any site that isn't in the comp should be the expected lowest allele frequency
                    }
                }
                switch(scale) {
                    case LINEAR:
    ...
    private int getLogitBucket(double alleleFrequency) {
        ...
        float score = (float)(-Math.log((alleleFrequency/(1-alleleFrequency))));

        // find correct bucket
        return Math.min(LOGIT_LIMIT, Math.max(-LOGIT_LIMIT, Math.round(score)));
    }
}
