...
public final class QualByDepth extends InfoFieldAnnotation implements StandardAnnotation, ReducibleAnnotation {
    ...
    public String getRawKeyName() {
        return GATKVCFConstants.RAW_QUAL_APPROX_KEY;
    }
    ...
    public Map<String, Object> combineRawData(List<Allele> allelesList, List<ReducibleAnnotationData<?>> listOfRawData) {
        ...
        for (final ReducibleAnnotationData currentValue : listOfRawData) {
            ...
            int combinedValue = 0;
            if (combinedData.getAttribute(Allele.NO_CALL) != null) {
                combinedValue += Integer.parseInt(combinedData.getAttribute(Allele.NO_CALL).toString());
            }
            combinedData.putAttribute(Allele.NO_CALL, combinedValue +
                    Integer.parseInt(currentValue.getAttribute(Allele.NO_CALL).toString()));
        }
        final Map<String, Object> annotations = new HashMap<>();
        annotations.put(getRawKeyName(), combinedData.getAttribute(Allele.NO_CALL));
        return annotations;
    }

    private void parseRawDataString(ReducibleAnnotationData<Integer> myData) {
        myData.putAttribute(Allele.NO_CALL, Integer.parseInt(myData.getRawData()));
    }
