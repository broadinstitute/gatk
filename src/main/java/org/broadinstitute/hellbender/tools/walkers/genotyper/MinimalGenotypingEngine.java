 ...
 */
public class MinimalGenotypingEngine extends GenotypingEngine<UnifiedArgumentCollection> {
    ...
    public MinimalGenotypingEngine( final UnifiedArgumentCollection configuration,
        ...
        if ( configuration.genotypingOutputMode == GenotypingOutputMode.GENOTYPE_GIVEN_ALLELES ) {
            throw new UserException("GENOTYPE_GIVEN_ALLELES mode not supported in the UnifiedGenotypingEngine");
        }

        if ( configuration.GLmodel != GenotypeLikelihoodsCalculationModel.Model.SNP ) {
            throw new UserException("Only the diploid SNP model is supported in the UnifiedGenotypingEngine");
        }

        if ( configuration.COMPUTE_SLOD ) {
            throw new UserException("--computeSLOD not supported in the UnifiedGenotypingEngine");
        }
    }
