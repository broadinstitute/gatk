...
public class EstimateDragstrParameters extends GATKTool {
    ...
    private List<SimpleInterval> sortAndMergeOverlappingIntervals(final List<SimpleInterval> input, final SAMSequenceDictionary dictionary) {
        if (isSortedAndHasNoOverlap(input, dictionary)) {
            ...
            final Map<String, List<SimpleInterval>> byContig = IntervalUtils.sortAndMergeOverlappingIntervals(input);
    ...
    private static class DragstrLocusCaseCollector implements Collector<org.broadinstitute.hellbender.utils.read.GATKRead, DragstrLocusCaseCollector, DragstrLocusCase> {
        ...
        private void collect(final org.broadinstitute.hellbender.utils.read.GATKRead read) {
            ...
            if (readStart <= paddedStrStart && readEnd >= paddedStrEnd) {
                ...
                final int mappingQuality = DragstrUtils.extendedMappingQuality(read);
                minMQ = Math.min(minMQ, mappingQuality);
