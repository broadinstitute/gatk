...
public class SplitReadEvidenceTester {
    ...
    protected SplitReadSite testSplitReadSite(final List<SplitReadEvidence> sortedEvidence,
        ...
        if (sortedEvidence.isEmpty()) {
            final EvidenceStatUtils.PoissonTestResult result = EvidenceStatUtils.calculateOneSamplePoissonTest(
                    Collections.emptyMap(), carrierSamples, backgroundSamples, sampleCoverageMap, PESREvidenceTester.DEPTH_BASIS
            );
        ...
        for (int i = 0; i < sortedEvidence.size(); i++) {
            ...
            if (i == sortedEvidence.size() - 1 || sortedEvidence.get(i + 1).getStart() != position) {
                final EvidenceStatUtils.PoissonTestResult result = EvidenceStatUtils.calculateOneSamplePoissonTest(
                        sampleCounts, carrierSamples, backgroundSamples, sampleCoverageMap, PESREvidenceTester.DEPTH_BASIS
                );
    ...
    public SVCallRecord applyToRecord(final SVCallRecord record,
        ...
        final Double firstQuality = refinedFirstSite.getP() == null || Double.isNaN(refinedFirstSite.getP()) ? null : EvidenceStatUtils.probToQual(refinedFirstSite.getP(), (byte) MAX_QUAL);
        final Double secondQuality = refinedSecondSite.getP() == null || Double.isNaN(refinedSecondSite.getP()) ? null : EvidenceStatUtils.probToQual(refinedSecondSite.getP(), (byte) MAX_QUAL);
        final Double totalQuality = Double.isNaN(bothsideResult.getP()) ? null : EvidenceStatUtils.probToQual(bothsideResult.getP(), (byte) MAX_QUAL);
        ...
        final Double startCarrierSignal = EvidenceStatUtils.carrierSignalFraction(refinedFirstSite.getCarrierSignal(),
                refinedFirstSite.getBackgroundSignal());
        final Double endCarrierSignal = EvidenceStatUtils.carrierSignalFraction(refinedSecondSite.getCarrierSignal(),
                refinedSecondSite.getBackgroundSignal());
        final Double totalCarrierSignal = EvidenceStatUtils.carrierSignalFraction(bothsideResult.getCarrierSignal(),
