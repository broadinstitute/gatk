...
public final class BaseRecalibratorOptimizedFn extends DoFnWLog<AddContextDataToReadOptimized.ContextShard, RecalibrationTables> {
    ...
    private void apply(GATKRead originalRead, final ReferenceDataSource refDS, Iterable<? extends Locatable> knownLocations) {
        ...
        if (originalRead instanceof SAMRecordToGATKReadAdapter) {
            // we ship those without the header, put it back.
            SAMRecordToGATKReadAdapter r = (SAMRecordToGATKReadAdapter)originalRead;
            r.setHeader(header);
        }
        ...
        isDeletion = new int[isSNP.length];
        final int nErrors = calculateSNPInsertDelete(read, refDS, isSNP, isInsertion, isDeletion);
    ...
    private static int calculateSNPInsertDelete(final GATKRead read, final ReferenceDataSource ref, int[] snp, int[] isIns, int[] isDel) {
