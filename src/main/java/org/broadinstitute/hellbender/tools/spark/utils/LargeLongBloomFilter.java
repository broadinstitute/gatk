...
public class LargeLongBloomFilter implements QueryableLongSet {
    ...
    protected LargeLongBloomFilter(final Kryo kryo, final Input stream) {
        ...
        for (int i = 0; i < numFilters; i++) {
            filters.add(kryo.readObject(stream, LongBloomFilter.class));
        }
    ...
    private static int longHash(final long entryVal) {
        return (int)SVUtils.fnvLong64(0x1035e606, entryVal);
    }
    ...
    private void initialize(final long maxPartitionBytes, final long numElements, final double fpp) {
        ...
        //Number of total bytes we'd like the set of filters to use
        final long idealTotalBytes = LongBloomFilter.getOptimalNumberOfBytes(numElements, fpp);

        //Largest legal size given max partition size
        final int maxLegalPartitionBytes = LongBloomFilter.getLegalSizeBelow(maxPartitionBytes);

        //Number of partitions, making sure to apply ceiling so that the actual false positive prob >= fpp
        int partitions = (int) Math.ceil(idealTotalBytes / (double) maxLegalPartitionBytes);
        if (partitions < 1) {
            partitions = 1;
        }

        //Just need the number of long's going into each partition, then let the LongBloomFilter class
        // decide what the optimal size is, given fpp
        final long elementsPerPartition = (long) Math.ceil(numElements / (double) partitions);

        filters = new ArrayList<>(partitions);
