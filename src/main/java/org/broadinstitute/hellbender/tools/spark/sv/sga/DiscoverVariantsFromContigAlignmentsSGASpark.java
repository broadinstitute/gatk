    ...
    public static final class SGATextFormatAlignmentParser extends AlignedContigGenerator {
        ...
        public static JavaPairRDD<String, List<AlignmentInterval>> parseAndBreakAlignmentTextRecords(final JavaSparkContext ctx, final String pathToInputAlignments, final Logger toolLogger) {
            final JavaPairRDD<String, List<AlignmentInterval>> contigNameAndAlignments
                    ...
                    .mapValues(rawAlignments -> rawAlignments.stream().flatMap(oneInterval -> Utils.stream(GappedAlignmentSplitter.split(oneInterval, StructuralVariationDiscoveryArgumentCollection.DiscoveryStepConstants.GAPPED_ALIGNMENT_BREAK_DEFAULT_SENSITIVITY, oneInterval.cigarAlong5to3DirectionOfContig.getReadLength() + SVVariantDiscoveryUtils.getTotalHardClipping(oneInterval.cigarAlong5to3DirectionOfContig)))).collect(Collectors.toList()));
            if (toolLogger!=null) {
        ...
        public static Tuple2<String, List<AlignmentInterval>> parseTextFileAlignmentIntervalLines(final String textLine) {
            ...
            try {
                ...
                for (int i = 1; i < fields.length; ++i) {
                    ...
                    intervals.add(new AlignmentInterval(AlignAssembledContigsSpark.decodeStringAsSimpleInterval(intervalFields[1]), contigStart, contigEnd, TextCigarCodec.decode(intervalFields[2]), intervalFields[3].equals("+"), Integer.valueOf(intervalFields[4]), Integer.valueOf(intervalFields[5])));
                }
