...
public class KmerAdjacencyBuilder extends CommandLineProgram {
    ...
    @Argument(doc = "input assembly", fullName = "assembly-fasta")
    private String assemblyFastaFile;
    ...
    @Override protected Object doWork() {
        ...
        final List<FermiLiteAssembly.Contig> flaContigs = new ArrayList<>(contigs.size());
        for ( final Contig contig : contigs ) {
            flaContigs.add(new FermiLiteAssembly.Contig(contig.getSequence().getBytes(), null, 0));
        }
        ...
        for ( int contigId = 0; contigId != nContigs; ++contigId ) {
            ...
            final List<FermiLiteAssembly.Connection> connections =
                    new ArrayList<>(contig.getFirst().getConnections().size() + contig.getLast().getConnections().size());
            for ( final Connection conn : contig.getFirst().getConnections() ) {
                final ContigEnd contigEnd = contigEnds.get(conn.getKmer());
                connections.add(new FermiLiteAssembly.Connection(flaContigs.get(contigEnd.getContigId()),
                        kSize2-1, true, !contigEnd.isFirst()));
            }
            for ( final Connection conn : contig.getLast().getConnections() ) {
                final ContigEnd contigEnd = contigEnds.get(conn.getKmer());
                connections.add(new FermiLiteAssembly.Connection(flaContigs.get(contigEnd.getContigId()),
                        kSize2-1, false, !contigEnd.isFirst()));
            }
            flaContigs.get(contigId).setConnections(connections);
        }

        final FermiLiteAssembly fla =
                FermiLiteAssemblyHandler.reviseAssembly(new FermiLiteAssembly(flaContigs), true, true);

        final String imageFile = BwaMemIndex.getDefaultIndexImageNameFromFastaFile(assemblyFastaFile);
        BwaMemIndex.createIndexImageFromFastaFile(assemblyFastaFile, imageFile);
        try ( final BwaMemIndex index = new BwaMemIndex(imageFile);
            ...
            final List<List<BwaMemAlignment>> alignments =
                    aligner.alignSeqs(fla.getContigs(), FermiLiteAssembly.Contig::getSequence);
            final List<SAMSequenceRecord> seqRecs = new ArrayList<>();
            final String assemblyFastaFileIndex = assemblyFastaFile + ".fai";
            try ( final BufferedReader rdr =
                          new BufferedReader(new InputStreamReader(BucketUtils.openFile(assemblyFastaFileIndex))) ) {
                String indexLine;
                while ( (indexLine = rdr.readLine()) != null ) {
                    final StringTokenizer toks = new StringTokenizer(indexLine);
                    final String name = toks.nextToken();
                    final int len = Integer.valueOf(toks.nextToken());
                    seqRecs.add(new SAMSequenceRecord(name, len));
                }
            } catch ( final IOException ioe ) {
                throw new UserException("Can't open assembly fasta index file " + assemblyFastaFileIndex);
            }
            final SAMSequenceDictionary seqDict = new SAMSequenceDictionary(seqRecs);
            final SAMFileHeader header = new SAMFileHeader(seqDict);
            header.setSortOrder(SAMFileHeader.SortOrder.queryname);

            final List<String> refNames = SequenceDictionaryUtils.getContigNamesList(seqDict);
            final Stream<SAMRecord> samRecordStream =
                    IntStream.range(0, fla.getNContigs())
                        .boxed()
                        .flatMap(idx ->
                                BwaMemAlignmentUtils.toSAMStreamForRead(String.format("na%05d", idx),
                                                                        fla.getContigs().get(idx).getSequence(),
                                                                        alignments.get(idx),
                                                                        header, refNames, null));
            SVFileUtils.writeSAMFile(graphOutput + ".bam", samRecordStream.iterator(), header, true);
        }
...
*/
    ...
    private static void dumpDOT( final List<Contig> contigs, final Map<SVKmerLong, ContigEnd> contigEnds, final String fileName ) {
    ...
    private static void dumpGFA( final List<Contig> contigs, final Map<SVKmerLong, ContigEnd> contigEnds, final String fileName ) {
        try ( final BufferedWriter writer = new BufferedWriter(new FileWriter(fileName)) ) {
            for ( final Contig contig : contigs ) {
