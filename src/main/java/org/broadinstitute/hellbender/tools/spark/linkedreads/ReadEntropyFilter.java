...
public class ReadEntropyFilter extends ReadFilter {
    ...
    public static double computeEntropy(String sequence) {
        final Map<SVKmer, Long> frequencies = SVKmerizer.stream(sequence, 3, new SVKmerShort()).collect(Collectors.groupingBy(k -> k, Collectors.counting()));
