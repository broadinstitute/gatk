...
public class InternalFilterLongReadAlignmentsSAMSpark extends GATKSparkTool {
    ...
    protected void runTool(final JavaSparkContext ctx) {
        ...
        oldWayOfFiltering(reads, header, localLogger)
                .coalesce(1).saveAsTextFile(outputFilePrefix+"_oldFiltering");

        newWayOfFiltering(reads, header, localLogger)
                ...
                .coalesce(1).saveAsTextFile(outputFilePrefix+"_newFiltering");
    }
     ...
     */
    static JavaRDD<AlignedContig> newWayOfFiltering(final JavaRDD<GATKRead> longReads,
                                                    final SAMFileHeader header,
                                                    final Logger toolLogger) {
    ...
    /**
     * Manages new filtering, then split the gapped alignments by delegating to {@link GappedAlignmentSplitter}.
     */
    @VisibleForTesting
    static JavaRDD<AlignedContig> filterAndSplitGappedAI(final JavaRDD<AlignedContig> parsedContigAlignments,
                                                         final Set<String> primaryRefContigs) {

        return parsedContigAlignments
                .mapToPair(alignedContig -> new Tuple2<>(alignedContig.contigName,
                        new Tuple2<>(alignedContig.contigSequence, newFiltering(alignedContig, primaryRefContigs))))
                .flatMap(InternalFilterLongReadAlignmentsSAMSpark::reConstructContigFromPickedAlignments)
                .sortBy(tig -> tig.contigName, true, parsedContigAlignments.getNumPartitions());
    }

    /**
     * Filters out alignments of a contig based on a heuristic scoring scheme implemented in
     * {@link #computeScoreOfConfiguration(List, Set, int)}.
     */
    @VisibleForTesting
    static List<List<AlignmentInterval>> newFiltering(final AlignedContig alignedContig,
                                                      final Set<String> primaryRefContigs) {

        // speed up if number of alignments is too high
        final int maxPrimaryAlignerScore = alignedContig.alignmentIntervals.stream()
                .filter(alignmentInterval -> primaryRefContigs.contains(alignmentInterval.referenceSpan.getContig()))
                .mapToInt(ai -> ai.alnScore).max().orElse(0); // possible that no mapping to primary
        ...
        if (alignedContig.alignmentIntervals.size() > 10) {
            alignmentIntervals = alignedContig.alignmentIntervals.stream()
                    .filter(alignmentInterval -> (!primaryRefContigs.contains(alignmentInterval.referenceSpan.getContig())
                                                                           && alignmentInterval.alnScore > maxPrimaryAlignerScore)
        ...
        final List<List<AlignmentInterval>> allConfigurations = Sets.powerSet(new HashSet<>(alignmentIntervals))
                .stream().map(ArrayList::new).map(ls -> ls.stream().sorted(AlignedContig.sortAlignments()).collect(Collectors.toList())) // make sure the order is sorted
                .collect(Collectors.toList());

        final int newMaxPrimaryAlignerScore = alignmentIntervals.stream()
                .filter(alignmentInterval -> primaryRefContigs.contains(alignmentInterval.referenceSpan.getContig()))
                .mapToInt(ai -> ai.alnScore).max().orElse(0); // possible that no mapping to primary

        final List<Double> scores = allConfigurations.stream()
                .map(configuration -> computeScoreOfConfiguration(configuration, primaryRefContigs, newMaxPrimaryAlignerScore))
                .collect(SVUtils.arrayListCollector(allConfigurations.size()));

        final double max = scores.stream().mapToDouble(Double::doubleValue).max()
                .orElseThrow(() -> new GATKException("Cannot find best score for alignments of contig: " + alignedContig.contigName));
    ...
    static double computeScoreOfConfiguration(final List<AlignmentInterval> configuration,
                                              final Set<String> primaryRefContigs,
                                              final int maxPrimaryAlignerScore) {

        double tigExplainQual =
                configuration.stream().mapToDouble(alignmentInterval -> {
                    final int len = alignmentInterval.endInAssembledContig - alignmentInterval.startInAssembledContig + 1;
                    final double factor;
                    if (primaryRefContigs.contains(alignmentInterval.referenceSpan.getContig())) {
                        factor = alignmentInterval.mapQual/60.0;
                    } else {
                        factor = Math.max(alignmentInterval.mapQual/60.0, (alignmentInterval.alnScore > maxPrimaryAlignerScore ? 1 : 0));
                    }
                    return factor * len;
                })
                        .sum();
     ...
     */
    private static Iterator<AlignedContig> reConstructContigFromPickedAlignments(
    ...
    private static List<AlignmentInterval> splitGaps(final List<AlignmentInterval> configuration) {
        return configuration.stream()
                .map(ai -> GappedAlignmentSplitter.split(ai, GAPPED_ALIGNMENT_BREAK_DEFAULT_SENSITIVITY,
                        SVVariantDiscoveryUtils.getUnclippedReadLength(ai.cigarAlong5to3DirectionOfContig)))
     ...
     */
    static Set<String> getPrimaryRefContigs() {
        final List<String> first22ChromosomesNum = IntStream.range(0, 23).mapToObj(String::valueOf).collect(Collectors.toList());
        final Set<String> primaryRefContigs = first22ChromosomesNum.stream().map(name -> "chr" + name).collect(Collectors.toSet());
        primaryRefContigs.addAll(first22ChromosomesNum);
        primaryRefContigs.addAll(Arrays.asList("chrX", "chrY", "chrM", "X", "Y", "MT"));
        return primaryRefContigs;
    }
}
