...
public class TandemDuplicationFactory extends SimpleSVFactory {

    public TandemDuplicationFactory(final SVIntervalTree<EvidenceTargetLink> intrachromosomalLinkTree,
    ...
    protected SimpleSV getNewSV(final int start,
        ...
        return new SimpleSV(SimpleSVType.TYPES.DUP, start, end, contigId, contig, readPairEvidence, splitReadEvidence, readPairCounterEvidence, splitReadCounterEvidence,
    ...
    protected boolean supportedBySegmentCalls(final SVInterval interval, final Set<CalledCopyRatioSegment> overlappingSegments) {
        return overlappingSegments.stream().anyMatch(segment -> IntervalUtils.segmentCallMatchesInterval(segment, interval, CalledCopyRatioSegment.Call.AMPLIFICATION, readMetadata, arguments.MIN_SEGMENT_RECIPROCAL_OVERLAP));
    }
