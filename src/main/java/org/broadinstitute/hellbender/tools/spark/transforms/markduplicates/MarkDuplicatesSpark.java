...
public final class MarkDuplicatesSpark extends GATKSparkTool {
    ...
    public static JavaRDD<GATKRead> mark(final JavaRDD<GATKRead> reads, final SAMFileHeader header,
        ...
        return reads.zipPartitions(repartitionedReadNames, (readsIter, readNamesIter)  -> {
            final Map<String,Integer> namesOfNonDuplicateReadsAndOpticalCounts = Utils.stream(readNamesIter).collect(Collectors.toMap(Tuple2::_1,Tuple2::_2,Integer::max)); //TODO there is a bug here with value 0 being overloaded with entries, this is a stopgap for profiling
