...
public class LargeLongHopscotchSet implements Serializable, LargeQueryableLongSet {
    ...
    protected LargeLongHopscotchSet(final Kryo kryo, final Input stream) {
        ...
        sets = (ArrayList<LongHopscotchSet>) kryo.readClassAndObject(stream);
        numSets = sets.size();
    ...
    private static int longHash(final long entryVal) {
        return SVUtils.fnvLong(0x4c79ac6a, entryVal);
    }
    ...
    protected void serialize(final Kryo kryo, final Output stream) {
        ...
        kryo.writeClassAndObject(stream, sets);
