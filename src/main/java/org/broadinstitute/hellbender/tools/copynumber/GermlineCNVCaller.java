 ...
 *     (set by <code>--p-alt &lt;value&gt;</code>), prevalence of active (i.e. CNV-rich) intervals (set by
 *     <code>--p-active &lt;value&gt;</code>), the coherence length of CNV events and active/silent domains
 *     across intervals (set by <code>--cnv-coherence-length &lt;value&gt;</code> and <code>--class-coherence-length
 *     &lt;value&gt;</code>, respectively), and the typical scale of interval- and sample-specific unexplained variance
 *     (set by <code>--interval-psi-scale &lt;value&gt;</code> and <code>--sample-psi-scale &lt;value&gt;</code>,
 *     respectively). It is crucial to note that these hyperparameters are <em>not</em> universal and must be tuned
 *     for each sequencing protocol and properly set at runtime.</p></dd>
 *
