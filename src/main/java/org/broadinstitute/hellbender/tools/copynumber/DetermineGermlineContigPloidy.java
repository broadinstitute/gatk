 ...
 *
 * <p>
 *     <b>Important Remark:</b> The Bayesian model underlying this tool assumes integer ploidy states (in contrast
 *     to fractional/variable ploidy states). Therefore, the tool is to be used strictly for germline samples and
 *     for the purpose of sex genotyping and detecting germline aneuploidy in autosomal contigs. The presence of
 *     significant somatic events and mosaicism (e.g. sex chromosome loss and somatic trisomy)
 *     will naturally lead to unreliable results. We strongly recommended inspecting genotyping qualities (GQ)
 *     from the tool output and considering to drop low-GQ contigs in the downstream analyses. Finally, given
 *     the Bayesian status of this tool, we suggest including as many high-quality germline samples as possible
 *     when building a ploidy-model in the COHORT mode (see below). This will downplay the role of questionable
 *     samples and will yield a more reliable estimation of genuine sequencing biases.
 * </p>
 *
 * <p>This tool has two modes as described below:</p>
 * <dl>
 *     <dt>COHORT mode:</dt>
 *
 *     <dd>If a ploidy-model directory is not provided via the {@code model} argument, the tool is run in the COHORT
 *      mode. In this mode, ploidy-model parameters (e.g. coverage bias and variance for each contig) are inferred,
 *      along with baseline contig ploidy states of each sample. A tab-separated table specifying prior probabilities
 *      for each ploidy state and for each contig is required in this mode and must be specified via the
 ...
 *          </tr>
 *      </table>
 *      Note that the contig names under {@code CONTIG_NAME} column must match contig names in the counts
 *      files, and all contigs appearing in the counts files must have a corresponding entry in the priors table.
 *      The order of contigs is immaterial in this table. The highest ploidy state is determined by the prior table
 *      (3 in the above example). A ploidy state can be strictly forbidden by setting its prior probability to 0.
 *      For example, the X contig in the above example can only assume 0 and 1 ploidy states.
 *
 ...
 *      be used later on for ploidy determination in one or more similarly-sequenced samples (see below).
 *
 ...
 *      coverage variance estimates.</p>
 *     </dd>
 *
 *     <dt>CASE mode:</dt>
 *     <dd>If a directory containing previously inferred ploidy-model parameters is provided via the {@code model}
 *      argument, then the tool is run in the CASE mode. In this mode, the ploidy-model parameters are loaded from the
 *      provided directory and only sample-specific quantities are inferred. Subsequently, the output directory will
 *      only contain the "-calls" subdirectory.
 *
