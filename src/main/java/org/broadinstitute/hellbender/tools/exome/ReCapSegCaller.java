...
public final class ReCapSegCaller {
    ...
    public static void makeCalls(ExonCollection<TargetCoverage> targets, final List<Segment> segments) {
        ...
        final double[] nearNeutralCoverage = segments.stream()
                .filter(s -> Math.abs(s.mean(targets) - neutralCoverage) < COPY_NEUTRAL_CUTOFF)
                .flatMap(s -> s.overlappingTargets(targets).stream())
                .mapToDouble(TargetCoverage::getCoverage)
                .toArray();
        ...
        for (final Segment segment : segments) {
            ...
            final double Z = (segment.mean(targets) - neutralCoverage)/neutralSigma;
