...
public class ACSModeledSegmentUtils {
    ...
    public static void writeACNVModeledSegmentFileAsAllelicCapSegFile(final File outFile,
                                                                      final List<AllelicSplitCall> allelicSplitCalls,
                                                                      final Genome genome) {
        Utils.nonNull(genome, "The genome cannot be null.");
        Utils.nonNull(allelicSplitCalls, "Calls cannot be null.");

        final List<ACSModeledSegment> acsModeledSegments = convertACNVSegmentsToACSSegments(allelicSplitCalls, DIVISOR, genome);
     ...
     *
     * @param allelicSplitCalls segments and calls generated by CNLoHCaller.  Never {@code null}
     ...
     */
    public static List<ACSModeledSegment> convertACNVSegmentsToACSSegments(final List<AllelicSplitCall> allelicSplitCalls, final double divisor, final Genome genome) {
        ParamUtils.isPositive(divisor, "Divisor must be greater than 0.");
        Utils.nonNull(allelicSplitCalls);
        Utils.nonNull(genome);

        return allelicSplitCalls.stream().map(c -> convertACNVSegmentToACSSegment(c.getAcnvSegment(), divisor, genome, c.getBalancedCall() == AllelicBalanceCall.BALANCED))
