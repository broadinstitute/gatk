...
public class FilterByOrientationBias extends VariantWalker {
    ...
    public Object onTraversalSuccess() {
        ...
        final Path output = IOUtils.getPath(outputVcf);
        final String filename = output.getFileName().toString();
        final Path summary = output.resolveSibling(filename + SUMMARY_FILE_SUFFIX);

        OrientationBiasUtils.writeOrientationBiasSummaryTable(
            sampleTransitionCombinations,
            finalVariants,
            transitionToPreAdapterScoreMap,
            summary.toFile());
        return null;
    }
