...
RUN echo "source activate gatk" > /root/run_unit_tests.sh && \
    ...
    echo "cp /jars/com/intel/gkl/NativeLibraryLoader.class" >> /root/run_unit_tests.sh && \
    echo "jar tvf \$TEST_JAR | grep gkl" >> /root/run_unit_tests.sh && \
    echo "jar uf ./gatk.jar com/intel/gkl/NativeLibraryLoader.class" >> /root/run_unit_tests.sh && \
