...
Each sample's Z-score coverage is segmented and called separately via the Viterbi algorithm, which finds the maximum-likelihood solution of an HMM.  The hidden states are neutral, deletion, and duplication -- the XHMM model does not take into account homozygous deletions or multiple duplications.  The input parameters are: (i) the probability $p$ that a target is the start of an event; (ii) the mean number $1/q$ of targets in a CNV; and (iii) the mean distance $D$ between targets in a CNV.  Note that the latter two of these are silly because they depend on the density of targets used.  In particular parameters that are tailored for an exome of 100,000 targets are completely wrong for a small gene panel.  Furthermore, if the density of targets is not uniform across the genome, these parameters might be appropriate only in some parts of the genome.  A much more appropriate, biological, parameter would be the average length (in base pairs) of a CNV.

Anyway, the HMM transition matrix is defined in terms of these parameters as follows.  We let $f = e^{-d/D}$ be a CNV memory attenuation factor between consecutive targets separated by distance $d$.  The transition matrix is
%
\begin{equation}
\bordermatrix{ {\rm from} \backslash {\rm to} & - & 0 & + \cr
      - & f(1-q) + (1-f)p & fq+(1-f)(1-2p) & (1-f)p \cr
      0 & p & 1-2p & p \cr
      + & (1-f)p & fq+(1-f)(1-2p) & f(1-q)+(1-f)p }
\end{equation}
%
The middle row simply states the definition of $p$ as the probably of starting an event.  The $- \rightarrow -$ and $+ \rightarrow +$ entries state, when $f=1$, the definition of $q$, and when $f=0$ the memoryless probability to start a new event.  For intermediate $f$ this element is a weighted average of the contributions with and without memory for continuing the CNV.  All remaining entries can be understood similarly.

The emission model is as follows.  Each hidden state emits a normally-distributed Z-score.  The means are $-M$, 0, and $+M$ for deletion, neutral, and duplication states, respectively, where $M$ is s user-specified parameter whose default is 3.  Each emission distribution is given unit variance.  This model is quite wrong.  Consider a duplication.  The tangent-normalized coverage ought be be roughly 0.5 times the proportional coverage -- the raw coverage is $3/2$ that of a diploid target, leaving $1/2$ remaining after (ideal) tangent-normalization.  Then division by the target standard deviaton to get a Z-score yields who-knows-what.  Since different targets have different average proportional coverage, the global parameter $M$ is misguided.  Basically, the current model is not a model at all, but a heuristic.
